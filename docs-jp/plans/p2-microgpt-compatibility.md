...
- 2026-02-22: `P2-MGPT-04` を実装。`microgpt/microgpt-20260222.py` を self_hosted parser / runtime 制約に合わせて `microgpt-lite` へ再構成し、`random.choices` / `random.gauss` / `random.shuffle` を利用した training/inference 導線を維持。`python3 src/py2cpp.py microgpt/microgpt-20260222.py -o out/microgpt-20260222.cpp` と `g++ -std=c++20 -I src -I src/runtime/cpp -fsyntax-only out/microgpt-20260222.cpp` の通過を確認した。
