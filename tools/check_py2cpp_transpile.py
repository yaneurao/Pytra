#!/usr/bin/env python3
"""Compatibility wrapper for check_py2cpp_transpile; delegates to check_py2x_transpile.py."""
...
def main() -> int:
    ap = argparse.ArgumentParser(description="compat wrapper: check_py2cpp_transpile")
    ...
    ap.add_argument("--check-multi-file-imports", action="store_true", help="forward to unified checker (cpp only)")
    ap.add_argument("--check-yanesdk-smoke", action="store_true", help="forward to unified checker (cpp only)")
    args = ap.parse_args()
    ...
    if args.check_multi_file_imports:
        cmd.append("--check-multi-file-imports")
    if args.check_yanesdk_smoke:
        cmd.append("--check-yanesdk-smoke")
