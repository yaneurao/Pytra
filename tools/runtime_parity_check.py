...
def build_targets(case_stem: str) -> list[Target]:
    return [
        Target(
            name="cpp",
            transpile_cmd=f"python src/py2cpp.py test/py/{case_stem}.py test/cpp/{case_stem}.cpp",
            run_cmd=(
        ...
        Target(
            name="rs",
            transpile_cmd=f"python src/py2rs.py test/py/{case_stem}.py test/rs/{case_stem}.rs",
            run_cmd=f"rustc -O test/rs/{case_stem}.rs -o test/obj/{case_stem}_rs.out && test/obj/{case_stem}_rs.out",
            needs=("python", "rustc"),
        ),
        Target(
            name="cs",
            transpile_cmd=f"python src/py2cs.py test/py/{case_stem}.py test/cs/{case_stem}.cs",
            run_cmd=(
                f"mcs -out:test/obj/{case_stem}_cs.exe test/cs/{case_stem}.cs "
                "src/cs_module/py_runtime.cs src/cs_module/time.cs src/cs_module/png_helper.cs src/cs_module/pathlib.cs "
                f"&& mono test/obj/{case_stem}_cs.exe"
            ),
            needs=("python", "mcs", "mono"),
        ),
        Target(
            name="js",
            transpile_cmd=f"python src/py2js.py test/py/{case_stem}.py test/js/{case_stem}.js",
            run_cmd=f"node test/js/{case_stem}.js",
            needs=("python", "node"),
        ),
        Target(
            name="ts",
            transpile_cmd=f"python src/py2ts.py test/py/{case_stem}.py test/ts/{case_stem}.ts",
            run_cmd=f"npx -y tsx test/ts/{case_stem}.ts",
            needs=("python", "node", "npx"),
        ),
        Target(
            name="go",
            transpile_cmd=f"python src/py2go.py test/py/{case_stem}.py test/go/{case_stem}.go",
            run_cmd=f"go run test/go/{case_stem}.go",
            needs=("python", "go"),
        ),
        Target(
            name="java",
            transpile_cmd=f"python src/py2java.py test/py/{case_stem}.py test/java/{case_stem}.java",
            run_cmd=f"javac test/java/{case_stem}.java && java -cp test/java {case_stem}",
            needs=("python", "javac", "java"),
        ),
        Target(
            name="swift",
            transpile_cmd=f"python src/py2swift.py test/py/{case_stem}.py test/swift/{case_stem}.swift",
            run_cmd=f"swiftc test/swift/{case_stem}.swift -o test/obj/{case_stem}_swift.out && test/obj/{case_stem}_swift.out",
            needs=("python", "swiftc", "node"),
        ),
        Target(
            name="kotlin",
            transpile_cmd=f"python src/py2kotlin.py test/py/{case_stem}.py test/kotlin/{case_stem}.kt",
            run_cmd=(
                f"kotlinc test/kotlin/{case_stem}.kt -include-runtime -d test/obj/{case_stem}_kotlin.jar "
                f"&& java -jar test/obj/{case_stem}_kotlin.jar"
            ),
...
def check_case(case_stem: str) -> int:
    py = run_shell(f"python test/py/{case_stem}.py")
    if py.returncode != 0:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="case stems under test/py (without .py)",
    )
    args = parser.parse_args()

    exit_code = 0
    for stem in args.cases:
        if not (ROOT / "test" / "py" / f"{stem}.py").exists():
