...
- Python 標準 `ast` 依存を廃し、セルフホスティング可能。(Pythonで書かれたトランスパイラ自分自身のコードを自分自身でC++のコードに変換できる。)
- ターゲットとなるPythonのソースコードの構文は、標準Pythonから文法などを拡張していないので、VS Codeなどで容易に開発ができる。
- C++のコードに変換したとき、C++のネイティブコードと同等の速度がでることを目指している。つまり、Pythonで書いて、C++と同等の速度が出せる(かも知れない)ということ。
...
- 開発背景・狙いを確認する: [docs/development-motivation.md](docs/development-motivation.md)
