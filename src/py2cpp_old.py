...
class CppTranspiler(BaseTranspiler):
    ...
    def transpile_module(self, module: ast.Module) -> str:
        ...
        include_lines: Set[str] = {
            ...
            '#include "cpp_module/py_runtime_old.h"',
        }
        ...
        if isinstance(stmt.targets[0], ast.Tuple):
            ...
            for elt in tuple_target.elts:
                if isinstance(elt, ast.Name):
                    ...
                    lines.append(f"{self.transpile_expr(elt)} = std::get<{i}>({tmp_name});")
                else:
    ...
    def _transpile_call(self, call: ast.expr) -> str:
        ...
        if isinstance(call.func, ast.Attribute):
            ...
            # NOTE:
            # `x.add(y)` is not globally rewritten here because it breaks
            # user-defined class methods named `add` (e.g. case24_class_static).
            # If set.add support is needed, it should be resolved with container
            # type information, not by method name alone.
