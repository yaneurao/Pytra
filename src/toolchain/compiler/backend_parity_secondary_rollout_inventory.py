...
SECONDARY_ROLLOUT_BUNDLES_V1: Final[tuple[SecondaryRolloutBundle, ...]] = (
    {
        ...
        "notes": "Start with the JVM-plus-Go bundle because it shares tuple/lambda/comprehension/iterator gaps while letting Java keep the already-reviewed for_range and range rows out of scope.",
    },
    {
        ...
        "target_evidence": "transpile_smoke",
        "notes": "Close with the static-runtime bundle; Scala/Swift still need for_range+range, while Nim swaps those for virtual_dispatch.",
    },
)
...
def build_secondary_rollout_handoff_manifest() -> dict[str, object]:
    return {
        ...
        "next_bundle": SECONDARY_ROLLOUT_HANDOFF_V1["next_bundle"],
