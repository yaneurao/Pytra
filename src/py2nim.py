...
from hooks.nim.emitter.nim_emitter import load_nim_profile, transpile_to_nim
...
def _copy_nim_runtime(output_path: Path) -> None:
    ...
    if not runtime_src.exists():
        # まだ runtime がない場合は空のファイルを作るか、警告を出す
        output_path.parent.mkdir(parents=True, exist_ok=True)
        runtime_dst = output_path.parent / "py_runtime.nim"
        runtime_dst.write_text("# Nim runtime placeholder\n", encoding="utf-8")
        return
    runtime_dst = output_path.parent / "py_runtime.nim"
...
def main() -> int:
    """CLI 入口。"""
    parser = argparse.ArgumentParser(description="Pytra EAST -> Nim transpiler")
    ...
    output_path.parent.mkdir(parents=True, exist_ok=True)
    nim_src = transpile_to_nim(east)
    output_path.write_text(nim_src, encoding="utf-8")
    _copy_nim_runtime(output_path)
    return 0
