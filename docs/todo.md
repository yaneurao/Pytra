...
3. [ ] selfhost コンパイルエラーを段階的にゼロ化する。
   ...
   - [x] 段階ゲート B: `total_errors <= 300` を達成し、同時に `test_code_emitter` 回帰を固定化。: 2026-02-18 再計測で `total_errors=256`、`test/unit/test_code_emitter.py` は 12/12 pass
...
- 更新（2026-02-18 selfhost 追加）:
  ...
  3. 現在の主因は `Any/object` 境界由来の C++ 型不整合（`selfhost/py2cpp.compile.log` で `total_errors=256`）。
