...
- 更新（2026-02-18 selfhost 追加）:
  ...
  3. 現在の主因は `Any/object` 境界由来の C++ 型不整合（`selfhost/build.all.log` で `total_errors=322`）。
  4. 先頭エラー群は `render_boolop` / 文字比較最適化ヘルパー（`_one_char_str_const`, `_str_index_char_access`）の `Any` 境界と、`CppEmitter` 生成シグネチャの不整合が中心。
