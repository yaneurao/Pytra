...
3. [ ] CodeEmitter hook 移管の再開（selfhostを壊さない手順）
   ...
   - [ ] `cpp_hooks.py` は最初に `png/gif` のみ移管し、`py2cpp.py` の既存分岐を1件ずつ削る。
     ...
     - [ ] `gif.save_gif` の解決ロジックを `cpp_hooks.py` へ移し、`py2cpp.py` 側の分岐を削除する。: `save_gif` 引数整形 helper 化・`runtime_call=save_gif` の hook 化は完了。`py2cpp.py` 直書き（Name fallback）の最終撤去が残り。
