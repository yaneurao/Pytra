...
4. [ ] フック注入 (`EmitterHooks`) を実装する。
   ...
   - [ ] `emit_stmt(If/While/For/AnnAssign/AugAssign)` の分岐を hooks + template helper へ段階分離する。: `If/While` は `_emit_if_stmt` / `_emit_while_stmt` へ分離済み、`for_range` / `for_each` ヘッダは `syntax_line` 化済み
