...
補足:
- 入力コードで使う Python モジュールに対応する実装は、canonical lane として `src/runtime/rs/{native,generated}/` に置いてください。checked-in `src/runtime/rs/pytra/` は delete target debt であり、runtime ownership root として扱ってはいけません。
