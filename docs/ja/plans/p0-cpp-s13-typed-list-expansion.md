<a href="../../en/plans/p0-cpp-s13-typed-list-expansion.md">
...
- 2026-03-01: `pyobj` でも concrete な `list[T]`（`Any/unknown/None` を含まない要素型）を typed list 候補として扱う判定を追加し、`sample/13` の `grid/stack/dirs/frames/candidates` を value-model 出力へ寄せた。`test_py2cpp_codegen_issues`（84件）・`test_east3_cpp_bridge`（90件）・`check_py2cpp_transpile`（134/134）・sample/13 parity（cpp）を通過。

## 分解

- [x] [ID: P0-CPP-S13-TYPED-LIST-EXPAND-01-S1-01] `cpp_list_model=pyobj` の typed list 判定拡張条件（concrete 要素型）を仕様化する。
- [x] [ID: P0-CPP-S13-TYPED-LIST-EXPAND-01-S2-01] emitter 実装を更新し、sample/13 の `grid/stack/dirs/frames` を typed list へ寄せる。
- [x] [ID: P0-CPP-S13-TYPED-LIST-EXPAND-01-S3-01] sample/13 断片回帰を追加し、transpile/check を通す。
