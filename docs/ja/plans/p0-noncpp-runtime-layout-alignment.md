...
- [ ] [ID: P0-NONCPP-RUNTIME-LAYOUT-ALIGN-01-S4-02] `generated/built_in/*` と `native/built_in/*` の責務境界を確定し、`py_runtime.*` に残る built-in 相当実装を縮退させる。あわせて C# の `pytra/**` duplicate lane（`math/time/json/pathlib/png/gif` など）を削除対象として整理する。
