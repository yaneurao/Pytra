...
- 2026-03-10 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02]: 次の slice で `src/toolchain/ir/core.py` に `_sh_make_expr_token()` / `_sh_make_import_binding()` を追加し、`ExprParser._tokenize()` と `_sh_append_import_binding()` の raw carrier を helper 正本へ寄せた。tracked generated mirror の `selfhost/runtime/cpp/pytra-gen/compiler/east_parts/core.cpp` にも同名 helper を追加して tokenizer/import-binding の open-coded `make_object` dict を畳み、`test_prepare_selfhost_source.py` に guard を足して再流入を防いだ。
