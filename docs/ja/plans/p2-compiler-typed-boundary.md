...
- 2026-03-10 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02]: その follow-up では `CompilerRootDocument::to_legacy_dict()` を `dict<str, str>` / `dict<str, int64>` から `dict<str, object>` への converting constructor ベースに寄せ、`src/runtime/cpp/native/compiler/transpile_cli.cpp` から explicit `make_object(...)` を消した。これで `transpile_cli.cpp` も既存の `backend_registry_static.cpp` と同じ zero-explicit-boxing 状態になり、native wrapper 側の boxing は `LayerOptionsCarrier::to_legacy_dict()` など adapter seam にさらに集中する。`test_py2x_entrypoints_contract.py` でも両ファイルの `make_object` 行数が 0 のまま維持されることを固定した。
