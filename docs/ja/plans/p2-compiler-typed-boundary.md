...
- 2026-03-09 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-01]: さらに `_sh_make_constant_expr()` を追加し、`_parse_comp_target()`、`_parse_primary()`、簡易 list-comp/tuple lower の `Name` / `Tuple` / `Constant` 生成を helper 経由へ揃えた。これで leaf-node 直組み立ての一部も builder seam 側へ寄り、残る大きな塊は expression envelope と `Subscript` / `Attribute` / `Call` まわりに絞られた。
