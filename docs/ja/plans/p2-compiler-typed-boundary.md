...
- 2026-03-10 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02]: 次の generated mirror slice では `selfhost/runtime/cpp/pytra-gen/compiler/east_parts/core.cpp` に `_sh_make_arg_node()` / `_sh_make_lambda_arg_entry()` / `_sh_make_keyword_arg()` / `_sh_make_cast_entry()` を追加し、`_parse_lambda()`・`_parse_postfix()`・`_make_bin()` に残っていた inline `make_object` dict を helper carrier へ寄せた。`test_prepare_selfhost_source.py` でも helper 定義と旧 keyword/cast literal の不在を固定したので、generated C++ lane でも同じ cluster の再流入を fail-fast できる。
...
- 2026-03-10 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02]: 続く generated mirror slice では `_sh_make_attribute_expr()` / `_sh_make_call_expr()` / `_sh_make_binop_expr()` を追加し、`_parse_postfix()` の `Attribute` / `Call` node と `_make_bin()` および f-string 連結で使う `BinOp` node を helper builder へ寄せた。`test_prepare_selfhost_source.py` でも旧 inline node literal を禁止したため、generated C++ lane の expression node cluster も helper 正本から外れて再流入しにくくなった。
