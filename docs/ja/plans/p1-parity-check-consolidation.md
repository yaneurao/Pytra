...
- 2026-04-25: `runtime_parity_check.py` を fast 実装へ委譲する薄い正規エントリへ置換。共有 helper は `runtime_parity_shared.py` に分離し、旧 subprocess 版の `build_targets` / `check_case` は削除した。`test_runtime_parity_check_cli.py` は fast 経路の `check_case` と共有 helper のテストへ整理した。
