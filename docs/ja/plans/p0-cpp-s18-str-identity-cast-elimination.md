...
- [ ] [ID: P0-CPP-S18-STR-IDENTITY-CAST-01] `str` 既知経路の `py_to_string` 同型変換を縮退する。
- [ ] [ID: P0-CPP-S18-STR-IDENTITY-CAST-01-S1-01] `str` 同型変換の現状箇所を棚卸しし、適用条件/除外条件を固定する。
- [ ] [ID: P0-CPP-S18-STR-IDENTITY-CAST-01-S2-01] emitter（必要なら EAST3 pass）へ縮退実装を追加し、`object` 経路は維持する。
- [ ] [ID: P0-CPP-S18-STR-IDENTITY-CAST-01-S2-02] sample/18 回帰を追加し、`py_to_string` 不要出力の再発を防止する。
- [ ] [ID: P0-CPP-S18-STR-IDENTITY-CAST-01-S3-01] transpile/unit/sample 再生成で非退行を確認する。
