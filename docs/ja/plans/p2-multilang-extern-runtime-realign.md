...
- 2026-03-14: `S2-01` の追加 bundle として、`std/math` に残っていた `rs_std_math_live_wrapper` / `php_std_math_live_wrapper` を `rs_math_runtime_wrapper` / `php_math_runtime_wrapper` へ改名し、manifest・generator・inventory・unit test の postprocess 名を generic math runtime wrapper seam に揃えた。
