...
- 2026-03-13: `S2-02` として、`tools/gen_runtime_from_manifest.py` の Wave A live regeneration を current tree で再現可能な状態へ戻した。Nim は `Try` を current checked-in helper artifact と同じ `# unsupported stmt: Try` comment lane で fail-soft に戻し、`utils/png.py` の `png_helper.nim` 再生成を通した。Java は string literal escape の不足を補い、`generated/std/json.java` の stale compare lane を解消した。結果として `gen_runtime_from_manifest.py --targets go,java,kotlin,scala,swift,nim --check` が通過する。
