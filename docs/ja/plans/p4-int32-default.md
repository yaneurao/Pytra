...
現状: `x: int8 = -1; y: uint16 = x` のように、符号付き負値を符号なし型に代入した場合、言語によって結果が異なる（C++: wraparound で 65535、Python: -1 のまま）。resolve は黙って通す。

対処方針:
- resolve 段で「符号付き型の変数を符号なし型に代入」するとき、元の値が負になり得る場合は **警告** を出す（エラーではない）
- リテラル代入（`y: uint16 = -1`）は静的に検出可能。変数代入（`y: uint16 = x` where `x: int8`）は型の組み合わせで検出
- parity check では言語間で結果が異なるため、fixture に signed 負値 → unsigned の直接代入パターンは含めない
