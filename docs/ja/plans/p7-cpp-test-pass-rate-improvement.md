...
- S2 残課題: `.h` と `.cpp` の list 型表現不統一。`pyobj_ref_lists` フラグが `.h`（header_builder）のみに適用され `.cpp`（emitter）と不整合。
  - `.h`: `pyobj_ref_lists=True` → `Object<list<T>>` で宣言
  - `.cpp`: emitter が `rc_list_ref()` で unwrap → `list<T>&` を渡す
  - 解決方針: runtime module の関数シグネチャは `list<T>&` で統一し、`Object<list<T>>` の wrap/unwrap は呼び出し側で行う。`.h` と `.cpp` の両方で同じ型表現を使う。
  - EAST 側の問題ではなく、C++ emitter / header_builder のリファクタ課題。
