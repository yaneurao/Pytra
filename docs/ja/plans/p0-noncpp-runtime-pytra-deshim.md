...
- 2026-03-13: S1-02 sixth bundle で `noncpp_runtime_layout_rollout_remaining_contract.py` の `compat_files` / `compat_modules` schema を `delete_target_files` / `delete_target_modules` へ改名した。まだ `target_roots=("generated","native","pytra")` と wave-B helper/iterator の `compat` 命名が残るため、S1-02 は継続する。
