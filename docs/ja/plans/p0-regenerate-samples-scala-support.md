<a href="../../en/plans/p0-regenerate-samples-scala-support.md">
...
背景:
- `sample/py` から `sample/scala` を更新する際、現状は `tools/gen/regenerate_samples.py` が `scala` を言語一覧に持たない。
- そのため Scala だけ再生成導線が分断され、`sample` 更新運用と回帰手順が他言語と不一致になっている。

目的:
- `tools/gen/regenerate_samples.py` で `--langs scala` を正式サポートし、`sample/scala` 再生成を既存導線へ統合する。
...
受け入れ基準:
- `python3 tools/gen/regenerate_samples.py --langs scala --force` が成功し、`sample/scala/*.scala` を一括再生成できること。
...
- [x] [ID: P0-REGEN-SCALA-SUPPORT-01-S2-01] `tools/gen/regenerate_samples.py` に `scala` 設定を追加し、CLI から `--langs scala` を受理できるようにする。
...
- 2026-03-02: [ID: P0-REGEN-SCALA-SUPPORT-01-S2-01] `tools/gen/regenerate_samples.py` に `scala` 設定（`cli=src/py2scala.py`, `out_dir=sample/scala`, `ext=.scala`）と `--langs` 既定値を追加し、`unknown language(s): scala` を解消した。
...
- 2026-03-02: [ID: P0-REGEN-SCALA-SUPPORT-01-S3-01] `python3 tools/gen/regenerate_samples.py --langs scala --force` を実行し、`summary: total=18 skip=0 regen=18 fail=0` を確認した。
