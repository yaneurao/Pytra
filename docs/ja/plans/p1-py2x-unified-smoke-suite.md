<a href="../../en/plans/p1-py2x-unified-smoke-suite.md">
...
背景:
- `tools/unittest/test_py2*_smoke.py` は CLI 経路・`--east-stage 2` 拒否・最小 fixture 変換など、共通の検証観点を各言語で重複実装している。
...
対象:
- `tools/unittest/test_py2*_smoke.py` に散在する共通 smoke 観点の棚卸し
- `test/unit` への共通 smoke テスト追加（target パラメタライズ）
- 各言語 smoke から共通化済みケースを削減し、固有ケースへ整理
- `tools/check/check_py2*_transpile.py` や既存 unit 実行導線との整合確認
...
- 対象: `tools/unittest/test_py2*_smoke.py` 14本（`cpp,rs,cs,js,ts,go,java,swift,kotlin,rb,lua,scala,php,nim`）
- 集計結果（関数名ベース）:
