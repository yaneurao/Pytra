...
- 2026-03-28: [ID: P3-COMMON-RENDERER-S4] C++ 側で `If/While` の condition hook と `BinOp/UnaryOp/Compare/BoolOp` の expr override を CommonRenderer 経由に戻しつつ、C++ 固有の truthiness / numeric promotion / runtime call 解決を維持するよう整理した。`pytra.utils.png/gif` の runtime EAST を toolchain2 経路で再生成し、`mapping.json` の `skip_modules` から `pytra.utils.` を外して `png.write_rgb_png` を native runtime symbol に解決するよう修正した。focused unittest と sample parity の `01_mandelbrot`, `02_raytrace_spheres`, `13_maze_generation_steps`, `14_raymarching_light_cycle` は C++ parity 通過を確認した。
