...
- テスト（少なくとも `tools/check/check_py2cpp_transpile.py` または `tools/unittest/test_py2cpp_*.py`）で、`pylib` 前提ケースが新規に必要でないことを確認する。
...
- [x] 2026-02-25: `P1-CPP-EMIT-NORM-01-S3` 実施。`pylib` 互換を検証対象としていた Cpp テストを除去し、`tools/unittest/test_py2cpp_features.py` の legacy 互換テスト `test_import_pylib_png_runtime` と `test/fixtures/imports/import_pylib_png.py` を削除。`docs/ja/spec/spec-dev.md` の既存記載（`pylib.runtime` 廃止）を明示要件として継続使用。
