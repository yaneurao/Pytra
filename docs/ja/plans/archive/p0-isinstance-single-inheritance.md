...
- `P0-ISINSTANCE-01-S4`: テスト再整理（`tools/unittest/*isinstance*`）と `check_todo` との対応。
...
- 2026-02-25: `P0-ISINSTANCE-01` Rust emitter を `py_isinstance` runtime API lower へ更新し、`type_id` 範囲テーブル（`PyTypeInfo`）を出力する helper 群を追加した。`tools/unittest/test_py2rs_smoke.py`（22件）、`tools/check/check_py2rs_transpile.py`（`checked=130 ok=130 fail=0 skipped=6`）、`tools/check/check_py2cpp_transpile.py`（`checked=131 ok=131 fail=0 skipped=6`）、`tools/check/check_py2cs_transpile.py`（`checked=130 ok=130 fail=0 skipped=6`）を確認。`tools/check/check_py2js_transpile.py` / `tools/check/check_py2ts_transpile.py` は `east3-contract` 既存失敗を回避して `--skip-east3-contract-tests` で `checked=130 ok=130 fail=0 skipped=6` を確認した。
- 2026-02-25: `P0-ISINSTANCE-01-S4` として `test_east3_cpp_bridge.py` の阻害失敗を解消（EAST3 strict は維持しつつ stage2/self_hosted 互換経路を限定許可、`dict[str,*]` key を `py_to_string` へ再統一、`render_cond(Any)` を `py_to_bool` へ再固定）。`tools/check/check_py2cpp_transpile.py`（`checked=131 ok=131 fail=0 skipped=6`）、`tools/check/check_py2rs_transpile.py` / `tools/check/check_py2cs_transpile.py` / `tools/check/check_py2js_transpile.py` / `tools/check/check_py2ts_transpile.py`（各 `checked=130 ok=130 fail=0 skipped=6`）を通常モードで確認し、`tools/unittest/test_py2{js,ts,cs,rs}_smoke.py`・`tools/unittest/test_js_ts_runtime_dispatch.py`・`tools/unittest/test_pytra_built_in_type_id.py` と `test_py2cpp_codegen_issues.py` の `isinstance` 5件を green 化した。
