...
- 2026-03-13: `S4-01` の current bundle として、`swift` native substrate に `__pytra_as_list` / `__pytra_as_dict` を追加し、`tools/gen_runtime_from_manifest.py` へ Swift の空 `@main` を剥がす `program_to_library` postprocess を追加した。これにより `contains/io_ops/iter_ops/predicates/sequence/zip_ops` を compile-safe subset として `src/runtime/swift/generated/built_in/*` へ materialize できるようになり、contract/checker と Swift build-run smoke を同期した。Wave A の compare-impossible backend set はこれで空になった。
