<a href="../../../en/plans/archive/p1-runtime-layout-unification.md">
...
|---|---|---|---|---|
| C# | `src/cs_module/{py_runtime.cs,pathlib.cs,png.cs,gif.cs,time.cs}` | `py2cs.py` は path 非依存。`cs_emitter.py` は import `module_id` を namespace 化し、生成コードは `Pytra.CsModule.*` を参照 | `src/runtime/cs/pytra/` 完了（`P1-RUNTIME-06-S1`） | runtime 実体を `src/runtime/cs/pytra/` へ移動済み。 |
| JS | `src/js_module/{py_runtime.js,pathlib.js,png.js,gif.js,math.js,time.js}` | `src/hooks/js/emitter/js_emitter.py`（S2 で新パスへ切替済み） | 新参照は `src/runtime/js/pytra/` 基準、旧 `src/js_module` は shim のみ | S3 で旧パス直参照を生成物/テストから撤去し、shim 運用へ縮退 |
| TS | `src/ts_module/{py_runtime.ts,pathlib.ts,png.ts,gif.ts,math.ts,time.ts}` | `py2ts.py` は `ts_emitter.py` 経由、`src/common2/js_ts_native_transpiler.py`（S2 で新パスへ切替済み） | 新参照は `src/runtime/ts/pytra/` 基準、旧 `src/ts_module` は shim のみ | S3 で旧パス直参照を生成物/テストから撤去し、shim 運用へ縮退 |
