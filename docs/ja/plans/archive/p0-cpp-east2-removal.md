<a href="../../../en/plans/archive/p0-cpp-east2-removal.md">
...
- 2026-02-24: [ID: `P0-CPP-EAST2-01-S3`] 回帰ガードを追加。`tools/check/check_py2cpp_transpile.py` で `--east-stage 2` が必ず失敗し、所定エラーメッセージを返すことを検査するように更新。`tools/unittest/test_py2cpp_smoke.py` を新設し、既定実行で互換警告が出ないことと `--east-stage 2` が拒否されることを固定した。`run_local_ci.py` に smoke を組み込んだ。
