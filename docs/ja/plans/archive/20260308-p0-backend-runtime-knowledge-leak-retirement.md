<a href="../../../en/plans/archive/20260308-p0-backend-runtime-knowledge-leak-retirement.md">
...
- [x] [ID: P0-BACKEND-RUNTIME-KNOWLEDGE-LEAK-01-S4-01] representative backend/test/tooling 回帰と guard を追加し、知識漏れの再侵入を防ぐ。
...
- 既存の positive/fail-closed 回帰としては、`tools/unittest/emit/cpp/test_py2cpp_features.py` の `math` import / alias / `png` / `gif`、`test_py2cpp_codegen_issues.py` の `save_gif` keyword order、`tools/unittest/emit/go/test_py2go_smoke.py` / `kotlin` / `swift` の `pyMath*` routing と source-scan guard、`tools/unittest/emit/php/test_py2php_smoke.py` と `tools/unittest/emit/scala/test_py2scala_smoke.py` の `save_gif` default + keyword order が代表ケースになる。
- tooling 側では `tools/unittest/tooling/test_runtime_symbol_index.py` が `math` / `png` canonical 解決、`tools/unittest/tooling/test_check_emitter_runtimecall_guardrails.py` が `write_rgb_png` の dispatch-table leakage を検知している。
...
- 2026-03-08: [ID: P0-BACKEND-RUNTIME-KNOWLEDGE-LEAK-01-S4-01] `tools/check/check_emitter_runtimecall_guardrails.py` を current contract に合わせて更新し、canonical runtime module / symbol 自体は許容しつつ、source-side module branch（`module_name == "math"` 等）、`binding_module.startswith("pytra.utils.")`、`runtime_symbol.startswith("pyMath")`、`resolved_runtime.endswith(".pi" | ".e")` だけを baseline 0 で検知するようにした。existing repo では allowlist なしで通ることを確認した。
- 2026-03-08: [ID: P0-BACKEND-RUNTIME-KNOWLEDGE-LEAK-01-S4-01] tooling 回帰として `test_check_emitter_runtimecall_guardrails.py` に source-module branch / `pyMath*` helper branch / canonical runtime module 許容の unit test を追加し、`test_runtime_symbol_index.py` には `lookup_runtime_symbol_doc("pytra.utils.gif", "save_gif")` と `resolve_import_binding_doc("pytra.utils", "gif", "symbol")` の direct assert を追加した。これで `gif` module leaf import と adapter metadata を backend 外で固定できる。
