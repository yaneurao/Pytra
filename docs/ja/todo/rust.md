...
1. [ ] [ID: P1-EMITTER-SELFHOST-RS-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/rs/cli.py --target cpp -o work/selfhost/emit/rs/` を実行し、変換が通るようにする
   - 進捗: 2026-04-29 に実行し、C++ 出力は途中まで進むが完走せず。`timeout 180s python3 src/pytra-cli.py -build src/toolchain/emit/rs/cli.py --target cpp -o work/selfhost/emit/rs/` は終了コード 124。停止時点で `work/selfhost/emit/rs/` は 28 ファイルの部分出力（うち C++ 10 件）に留まり、selfhost emitter のエントリ一式生成まで到達しない。
