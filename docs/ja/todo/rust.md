...
1. [x] [ID: P1-EMITTER-SELFHOST-RS-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/rs/cli.py --target cpp -o work/selfhost/emit/rs/` を実行し、変換が通るようにする
   ...
   - 完了: 2026-05-04 `PYTHONPYCACHEPREFIX=work/tmp/pycache timeout 3600s python3 src/pytra-cli.py -build src/toolchain/emit/rs/cli.py --target cpp -o work/selfhost/emit/rs` が PASS。`work/selfhost/emit/rs/` に 42 ファイルを生成し、linked manifest は 25 modules。
2. [x] [ID: P1-EMITTER-SELFHOST-RS-S2] 生成された C++ を `g++ -std=c++20 -O0` でコンパイルを通す（source 側の型注釈不整合を修正）
   - 完了: 2026-05-04 `PYTHONPYCACHEPREFIX=work/tmp/pycache timeout 3600s g++ -std=c++20 -O0 -w $(find work/selfhost/emit/rs -name '*.cpp' | sort) src/runtime/cpp/core/io.cpp src/runtime/cpp/std/pathlib.cpp src/runtime/cpp/std/sys.cpp src/runtime/cpp/std/os.cpp src/runtime/cpp/std/os_path.cpp src/runtime/cpp/std/glob.cpp -I work/selfhost/emit/rs -I src -I src/runtime/cpp -o work/selfhost/emit/rs/emitter_rs_cpp` が PASS。
