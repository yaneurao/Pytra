...
5. [x] [ID: P0-IMAGE-RUNTIME-CORE-GEN-01-S2-02] `tools/check/audit_image_runtime_sot.py` を「marker有無」から「`pytra-gen` 実体 + 生成痕跡 + core混入禁止」検査へ置換する。
...
- 進捗メモ: [ID: P0-IMAGE-RUNTIME-CORE-GEN-01-S2-01] `tools/gen/gen_image_runtime_from_canonical.py` を追加し、`png.py/gif.py -> src/runtime/<lang>/pytra-gen/...` の共通生成計画（14言語）を実装。`--dry-run` と `test_gen_image_runtime_from_canonical.py` で導線を検証。
- 進捗メモ: [ID: P0-IMAGE-RUNTIME-CORE-GEN-01-S2-02] `tools/check/audit_image_runtime_sot.py` を core/gen 検査へ置換（`pytra-core` 混入禁止、`pytra-gen` の `source:` / `generated-by:` 必須、legacy配置検知）。`image_runtime_core_gen_audit_20260304_s2_02.json` を取得し baseline（`compliant=1/non_compliant=13`）を固定。
...
- 進捗メモ: [ID: P0-IMAGE-RUNTIME-CORE-GEN-01-S4-03] `tools/check/audit_image_runtime_sot.py` に `--fail-on-core-mix` を追加し、`tools/run/run_local_ci.py` の必須ステップへ組み込み。`work/logs/image_runtime_core_gen_audit_20260304_s4_03_guardrail.json` で core 混入なし（fail条件未該当）を確認。
...
6. [x] [ID: P0-IMAGE-RUNTIME-STATIC-GUARDRAILS-01-S3-02] `tools/run/run_local_ci.py` と CI 導線に必須ジョブとして組み込む。
...
- 進捗メモ: [ID: P1-TEST-UNIT-LAYOUT-PRUNE-01-S1-02] `tools/unittest/{common,backends/<lang>,ir,tooling,selfhost}` の目標配置・命名・discover運用規約を計画書に確定し、`test/unit` 直下直置き禁止を明文化。
- 進捗メモ: [ID: P1-TEST-UNIT-LAYOUT-PRUNE-01-S2-01] `tools/unittest/test_*.py` 71本を責務別ディレクトリへ移動し、`run_local_ci.py` と `check_noncpp_east3_contract.py` の固定参照を新パスへ更新。`backends` 名衝突回避のため `tools/unittest/backends` は非package運用とした。
...
9. [x] [ID: P0-JAVA-PYRUNTIME-SOT-01-S4-02] 静的ガード（`PyRuntime.java` 禁止シンボル検査）を `tools/run/run_local_ci.py` へ組み込み、再発を fail-fast 化する。
...
- 進捗メモ: [ID: P0-JAVA-PYRUNTIME-SOT-01-S4-01] `PyRuntime.java` から `write_rgb_png/save_gif/grayscale_palette` 本体を削除し、`tools/check/check_java_pyruntime_boundary.py` で再混入禁止を fail-fast 化した（json/pathlib は残課題）。
...
- 進捗メモ: [ID: P0-JAVA-PYRUNTIME-SOT-01-S4-02] `tools/check/check_java_pyruntime_boundary.py` を `run_local_ci.py` へ組み込み済み。禁止シンボルは image 互換ラッパ + `pyPerfCounter` / `pyMath*` まで拡張した（json/pathlib は継続）。
...
- 進捗メモ: [ID: P0-EMITTER-RUNTIMECALL-GUARDRAILS-01-S4-05] `tools/check/check_java_runtimecall_api_boundary.py` を追加し、Java emitter の runtime-call API 境界違反（raw `expr` 受け渡し/生 callee 参照）を fail-fast 化、`run_local_ci.py` 必須導線へ組み込んだ。
