<a href="../../../en/todo/archive/20260222.md">
...
1. [x] `docs/ja/todo/index.md` の `ID: P0-SH-01` と `ID: P0-SH-03` を完了化した。
...
3. [x] 回帰確認を実施した。
   ...
   - [x] `python3 tools/check/check_selfhost_direct_compile.py --cases sample/py/*.py` で `failures=0` を確認した。
4. [x] 残課題の境界を明記した。
   - [x] `python3 tools/check/check_selfhost_cpp_diff.py --mode allow-not-implemented` は `mismatches=3`（`if_else.py`, `01_mandelbrot.py`, `04_monte_carlo_pi.py`）のままであり、差分縮退は `ID: P0-SH-04` 以降で継続する。

## 2026-02-22 完了: `render_expr` Call 分岐（builtin/module/method）分離タスクの完了反映

1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Split render_expr Call branches (builtin/module/method) by feature and migrate to CodeEmitter helpers` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Split arithmetic/comparison/type-conversion branches in render_expr into independent functions, switchable via profile/hooks` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Continue cleaning up unused functions` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Template control-flow branches in emit_stmt and move them to CodeEmitter.syntax_*` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Move shared templates for FunctionDef / ClassDef (open/body/close) into CodeEmitter` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: CodeEmitter / Hooks Migration` にある `render_expr(Call/BinOp/Compare)` の hooks + helpers 分離タスクを完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Override only C++-specific differences (brace omission, range-mode, etc.) in hooks` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: py2cpp Reduction` にある `Move base rendering of Constant(Name/Attribute) to shared CodeEmitter` を完了化した。
2. [x] 根拠確認を実施した。
...
1. [x] `docs/en/todo/index.md` の `P1: CodeEmitter / Hooks Migration` の `Implement hook injection (EmitterHooks)` を完了化した。
2. [x] 根拠確認を実施した。
...
3. [x] 回帰確認導線を追加した。
   - [x] `tools/unittest/test_py2cs_smoke.py` を追加した。
   - [x] `tools/check/check_py2cs_transpile.py` を追加し、`checked=117 ok=117 fail=0 skipped=5` を確認した。

## 2026-02-22 完了: `docs/ja/todo/index.md` の P3/P4 一式消化
...
1. [x] `P3: py2rs（EAST移行）を CodeEmitter 中心で再設計` を完了した。
   ...
   - [x] `py2rs` を EAST ベースで安定運用し、`tools/check/check_py2rs_transpile.py` で `checked=117 ok=117 fail=0 skipped=5` を確認した。
...
3. [x] 言語横断回帰を追加した。
   - [x] `tools/unittest/test_error_classification_cross_lang.py` を追加し、`load_east` のエラー分類メッセージ整合を固定化した。
4. [x] `docs/ja/todo/index.md` を未完了専用の最小構成に整理した。
...
3. [x] 成功条件を達成した。
   ...
   - [x] `tools/check/check_yanesdk_py2cpp_smoke.py` を追加し、`checked=8 ok=8 fail=0` を確認。
...
2. [x] `tools/check/check_py2cpp_transpile.py` に Yanesdk 縮小ケース群（`--check-yanesdk-smoke`）を追加した。
3. [x] `Yanesdk` 実ファイル smoke テストを追加した（`tools/check/check_yanesdk_py2cpp_smoke.py`）。
...
5. [x] 回帰テストを追加した。
   - [x] `tools/unittest/test_py2cpp_codegen_issues.py` の `test_ifexp_ternary_is_rendered_in_all_expression_positions`
...
11. [x] 回帰テストを追加した。
    - [x] `tools/unittest/test_east_core.py` に BOM/継続行/`**`/top-level Expr/class `pass`/nested def/tuple末尾`,`/`yield`/`;` 拒否のテストを追加。
    - [x] `test/fixtures/control/yield_generator_min.py` を追加し、Python/C++ 実行一致（`True`）を確認。

## 2026-02-22 完了: DOCS-SYNC-01 `todo/archive` 翻訳同期フロー整備

1. [x] `docs/ja/todo/index.md` の `ID: DOCS-SYNC-01` を完了した。
2. [x] `docs/en/todo/archive/` の運用導線を整備した。
   - [x] `tools/run/sync_todo_history_translation.py` を追加し、`docs/ja/todo/archive/YYYYMMDD.md` を基点に `docs/en/todo/archive/YYYYMMDD.md` の不足ファイル雛形を自動生成できるようにした。
   - [x] 同スクリプトで `docs/en/todo/archive/index.md` を自動更新し、日付別リンクと翻訳状態（`pending` / `done`）を可視化した。
   - [x] `python3 tools/run/sync_todo_history_translation.py --check` で file set の同期漏れ（missing/extra/index 欠落）を検出できるようにした。
3. [x] 文書ルールを同期した。
...
1. [x] `docs/ja/todo/index.md` の `ID: P1-MQ-08` を完了した。
...
1. [x] `docs/ja/todo/index.md` の `ID: P1-COMP-06` と `ID: P1-COMP-07` を完了した。
...
1. [x] `docs/ja/todo/index.md` の `ID: P1-COMP-08` を完了した。
...
1. [x] `docs/ja/todo/index.md` の `ID: P1-RUNTIME-04` と `ID: P1-RUNTIME-06` を完了した。
...
1. [x] `docs/ja/todo/index.md` の `ID: P0-SH-02` を完了した。
...
3. [x] 回帰確認を実施した。
   - [x] `python3 tools/build_selfhost.py` の成功を確認した。
   - [x] `python3 tools/check/check_selfhost_cpp_diff.py --mode allow-not-implemented --cases test/fixtures/control/if_else.py --show-diff` で、差分が「本文欠落」から「brace 整形差」に縮退したことを確認した。
   - [x] `python3 tools/check/check_selfhost_cpp_diff.py --mode allow-not-implemented --cases sample/py/01_mandelbrot.py --show-diff` / `--cases sample/py/04_monte_carlo_pi.py --show-diff` で、差分が range-mode/brace などの整形差に限定されることを確認した。
