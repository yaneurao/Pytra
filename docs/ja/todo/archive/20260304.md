...
7. [x] [ID: P0-PHP-IMAGE-RUNTIME-S16-01-S3-01] `tools/check/runtime_parity_check.py` でケース実行前の既存 artifact 削除を強制する。
...
- 進捗メモ: [ID: P1-PY2X-SINGLE-ENTRY-01-S3-01] `tools/check/check_legacy_cli_references.py` を追加し、`src/tools/test` の `src/py2*.py` 直参照・`import py2*` 新規流入を fail-fast 検出できるようにした。
...
- 進捗メモ: [ID: P1-PY2X-WRAPPER-REMOVE-REOPEN-01] archive からの差し戻し。`src/py2*.py` 実ファイルと wrapper 依存（`tools/check/check_multilang_selfhost_stage1.py`, `tools/check/check_noncpp_east3_contract.py`, `tools/unittest/test_py2*_smoke.py`）残存を確認し、完了判定を再オープンした。
