<a href="../../../en/todo/archive/20260226.md">
...
5. [x] [ID: P0-ISINSTANCE-01-S4] 既存 `isinstance` テストを更新・追加し、`tools/check/check_*_transpile.py` と `test/unit` の回帰条件を全言語で再固定化する。
...
- `P0-ISINSTANCE-01`: Rust emitter を `py_isinstance(&x, <type_id>)` lower + `type_id` 範囲テーブル出力へ移行し、`test_py2rs_smoke.py`（22件）/`tools/check/check_py2rs_transpile.py`（`checked=130 ok=130`）と `tools/check/check_py2{cpp,cs,js,ts}_transpile.py`（JS/TS は `--skip-east3-contract-tests` で `checked=130 ok=130`）で回帰を確認した。
- `P0-ISINSTANCE-01`: CppEmitter の EAST3 bridge 互換経路（legacy `isinstance`/builtin call/method、`dict[str,*]` key coercion、`render_cond(Any)`）を修正し、`tools/check/check_py2{cpp,rs,cs,js,ts}_transpile.py` と `isinstance` 関連 unit を通常モードで再固定化した。
...
3. [x] [ID: P0-SAMPLE-GOLDEN-ALL-01-S2] runtime parity 実行フローを全言語対応に整備し、`tools/check/runtime_parity_check.py` の到達条件（toolchain、入出力、失敗分類）を実運用で安定化する。
...
9. [x] [ID: P0-SAMPLE-GOLDEN-ALL-01-S8] 全言語最終結果を `docs/ja/README.md` / `readme.md` のサンプル実行結果へ反映し、`golden` 差分が発生しない運用を維持する。
...
- `P0-SAMPLE-GOLDEN-ALL-01-S7`: `go/java/swift/kotlin` を JS sidecar bridge + `swiftc` shim（`tools/shims/swiftc`）へ揃え、`runtime_parity_check.py --case-root sample --targets go,java,swift,kotlin --all-samples --ignore-unstable-stdout` で `SUMMARY cases=18 pass=18 fail=0` / `ok: 72` を確認した。
- `P0-SAMPLE-GOLDEN-ALL-01-S8`: `docs/ja/README.md` / `readme.md` の実行結果注記を更新し、全9言語の parity 完走（S3〜S7）を反映した。
...
4. [x] [ID: P1-CPP-PYTO-01-S3] `py_to_int64(object/any)` の例外・既定値挙動を検証し、`sample` や回帰テストで再生成差分が許容範囲かを確認して `docs/ja/README.md` の該当方針へ反映する。
...
- `P1-CPP-PYTO-01-S3`: `test_cpp_runtime_boxing.py` に `py_to_int64(object/any)` の既定値/strict 判定（`obj_to_int64_or_raise`）を追加し、`docs/ja/README.md` / `readme.md` へ「非変換値は 0、strict は `_or_raise`」方針を追記した。
...
13. [x] [ID: P0-SAMPLE-CPP-RS-PERF-01-S5-01] C++/Rust 18 件の再計測結果を `readme.md` / `docs/ja/README.md` へ反映する。
...
35. [x] [ID: P0-SAMPLE-CPP-RS-PERF-01-S16-01] 全18サンプルを再計測し、最新版の C++/Rust 実測値を `readme.md` / `docs/ja/README.md` に反映する。
...
38. [x] [ID: P0-SAMPLE-CPP-RS-PERF-01-S17-02] `16` 再計測と全18件再計測を実行し、`>1.5x` 外れ値が解消した最新版を `readme.md` / `docs/ja/README.md` と計画文書へ反映する。
...
9. [x] [ID: P2-CPP-SELFHOST-VIRTUAL-01-S4-02] `tools/check/check_selfhost_cpp_diff.py` と `tools/verify_selfhost_end_to_end.py` を再実行し、回帰条件が満たされることを確認する。
...
19. [x] [ID: P0-CPP-EMITTER-SLIM-01-S7-01] `tools/unittest/test_py2cpp_*.py` と `tools/check/check_py2cpp_transpile.py` を通し、分割後の回帰を固定する。
20. [x] [ID: P0-CPP-EMITTER-SLIM-01-S7-02] `tools/check/check_selfhost_cpp_diff.py` / `tools/verify_selfhost_end_to_end.py` を再実行し、selfhost 再変換可能性を確認する。
