...
- `P1-RS-RUNTIME-EXT-01-S3-02` `tools/gen/regenerate_samples.py --langs rs --force` で `sample/rs` 18件を再生成し、`py_runtime.rs` 以外に `fn py_perf_counter|fn py_isdigit|mod pytra {` が残っていないことを確認した。
...
- `P1-CORE-PATH-SOT-01-S3-02` `python3 tools/check/check_py2cpp_transpile.py` を再実行し、`checked=134 ok=134 fail=0 skipped=6` で非退行を確認した。
...
## 2026-03-01 移管: 完了済み子タスク（親 `P0-MULTILANG-INHERIT-DISPATCH-01` は継続）

以下の完了済み子タスクを `docs/ja/todo/index.md` から `todo/archive` へ移管。

文脈: [docs/ja/plans/p0-multilang-inheritance-dispatch-rollout.md](../plans/p0-multilang-inheritance-dispatch-rollout.md)

1. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S1-01] 追加 fixture `test/fixtures/oop/inheritance_virtual_dispatch_multilang.py` を backend smoke/parity 導線へ接続する。
2. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-CS] C# 改善計画 [p0-cs-inheritance-dispatch-uplift.md](../plans/p0-cs-inheritance-dispatch-uplift.md) を実施する。
3. [x] [ID: P0-MULTILANG-INHERIT-DISPATCH-01-S2-GO] Go 改善計画 [p0-go-inheritance-dispatch-uplift.md](../plans/p0-go-inheritance-dispatch-uplift.md) を実施する。

## 2026-03-01 移管: 完了済みセクション（6件）
