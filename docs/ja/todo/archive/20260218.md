...
- [x] 上記を `tools/unittest/test_py2cpp_features.py` の C++ 実行回帰に追加する。
...
- [x] C++ 変換して実行まで通る runtime test を追加する（`tools/unittest/test_py2cpp_features.py`）。
...
- [x] `tools/unittest/test_py2cpp_features.py` に `BuiltinCall` 正規化の回帰（`dict.get/items/keys/values`, `str` メソッド, `Path` メソッド）を追加する。
...
- [x] `return`（値なし）の再発防止 unit test（`tools/unittest/test_east_core.py`）を追加する。
