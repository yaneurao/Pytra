<a href="../../../en/todo/archive/20260224.md">
...
4. [x] [ID: P0-CPP-EAST2-01-S3] `tools/check/check_py2cpp_transpile.py` と `tools/unittest/test_py2cpp_smoke.py` を更新し、`EAST2` 経路が復活しない回帰ガードを追加する。
...
進捗メモ: [ID: P0-CPP-EAST2-01-S3] `check_py2cpp_transpile.py` に `--east-stage 2` 失敗必須チェックを追加し、`tools/unittest/test_py2cpp_smoke.py` を新設。`check_py2cpp_transpile` は `checked=132 ok=132 fail=0`。
...
1. [x] [ID: P0-EASTMIG-06-S4] `tools/unittest/test_east3_*` と `tools/check/check_py2*_transpile.py` を更新し、`EAST3` 主経路が回帰導線の既定になるよう固定する。
...
1. [x] [ID: P1-CED-04] `tools/check/check_selfhost_cpp_diff.py` で差分ゼロを維持しながら fallback を縮退する。
...
1. [x] [ID: P0-EASTMIG-03-S4] `tools/check/check_py2cpp_transpile.py` と `tools/check/check_selfhost_cpp_diff.py --mode allow-not-implemented` で主経路化後の回帰基線を固定する。
...
1. [x] [ID: P0-EASTMIG-05-S1] `tools/unittest/test_east3_lowering.py` / `tools/unittest/test_east3_cpp_bridge.py` を `EAST3` 主経路前提で拡充する。
...
1. [x] [ID: P0-EASTMIG-05-S2] `tools/check/check_py2{cpp,js,ts}_transpile.py` と selfhost 差分検証を `EAST3` 主経路の標準導線として文書化する。
...
1. [x] [ID: P1-CPP-REDUCE-02-S2] `tools/check/check_py2cpp_helper_guard.py` と `tools/py2cpp_cpp_helper_allowlist.txt` を追加し、`CppEmitter` private helper の新規追加を CI で検出できるようにする（`tools/run/run_local_ci.py` へ組み込み）。
...
1. [x] [ID: P1-COMP-05] `py2cpp.py` のトップレベル責務を「C++固有処理 + 互換ラッパ」に限定し、`tools/check/check_py2cpp_boundary.py` で境界を検証可能にする。CI 導線（`tools/run/run_local_ci.py`）へ同チェックを組み込み、将来の汎用処理逆流を防止する。
...
3. [x] [ID: P1-COMP-11] `src/pytra/compiler/transpile_cli.py` の汎用 helper 群を機能グループごとに `class + @staticmethod` へ整理し、`py2cpp.py` 側 import を class 単位へ縮退する。移行時はトップレベル互換ラッパーを暫定維持し、`tools/prepare_selfhost_source.py` / `tools/unittest/test_prepare_selfhost_source.py` の抽出ロジックも同時更新して selfhost 回帰を防ぐ。
