<a href="../../../en/todo/archive/20260219.md">
...
1. [x] `runtime/cpp/pytra/std/*` を自動生成へ移行する（`math` を含む）。
   ...
   - [x] `python3 tools/gen/generate_cpp_pylib_runtime.py --check` で `std/runtime` 両方の stale チェックが通ることを確認。
   - [x] 生成後に `tools/check/check_py2cpp_transpile.py` と `tools/verify_sample_outputs.py` で回帰確認する。
     - [x] `tools/check/check_py2cpp_transpile.py` は pass（`checked=103 ok=103 fail=0 skipped=5`）。
...
3. [x] import 解決機能の回帰テストを追加・強化する。
   - [x] `tools/unittest/test_py2cpp_features.py` に `pytra.std` / `pytra.runtime` import の include 生成テストを追加する。
...
作業ルール（step by step）:
- [x] `CodeEmitter` の変更は段階適用し、各ステップで `test/fixtures` の変換可否を確認してから次へ進む。: `tools/check/check_py2cpp_transpile.py` を追加
- [x] 同じく各ステップで `sample/py` 全件の変換可否を確認してから次へ進む。: `tools/check/check_py2cpp_transpile.py` を追加
...
5. [x] 回帰確認を追加する。
   - [x] `tools/unittest/test_code_emitter.py` を追加し、profile/hook の境界を検証する。
   - [x] `tools/unittest/test_py2cpp_features.py` と `tools/unittest/test_image_runtime_parity.py` を回帰する。
...
0. [x] `runtime/cpp/pylib` を完全自動生成へ移行する（手書きラッパ/手書きヘッダ廃止）
   ...
   - [x] 生成スクリプト `tools/gen/generate_cpp_pylib_runtime.py` を「ヘッダ/実装の両方を出力する」仕様へ拡張する。
   - [x] `tools/build_selfhost.py` / `tools/run/run_local_ci.py` / `tools/verify_*` の参照先を新生成物構成へ更新する。
   - [x] 受け入れ条件:
     - [x] `python3 tools/gen/generate_cpp_pylib_runtime.py --check` が pass。
     - [x] `python3 tools/check/check_py2cpp_transpile.py` が pass。
     ...
     - [x] `python3 tools/check/verify_image_runtime_parity.py` が pass。

4. [x] ローカル CI 相当手順の固定化
   - [x] 回帰コマンド群（transpile/feature/selfhost build/selfhost diff）を1コマンド実行できるスクリプト化。: `tools/run/run_local_ci.py`
...
5. [x] `unit` と `fixtures/stdlib` の同値性を揃える
   ...
   - [x] 既存 C++ ランタイムで実行可能な `math` / `pathlib` を `tools/unittest/test_py2cpp_features.py` の runtime 回帰に追加する。
   ...
   - [x] 未対応モジュールを順に C++ runtime 接続する（`json -> os/glob -> argparse -> sys/typing -> re -> dataclasses -> enum`）。
     - [x] `json`:
       ...
       - [x] `tools/unittest/test_py2cpp_features.py` に `json_extended` runtime 回帰を追加する。: `test_json_extended_runtime`
    - [x] `os/glob`:
      ...
      - [x] `tools/unittest/test_py2cpp_features.py` に `os_glob_extended` runtime 回帰を追加する。
     - [x] `argparse/sys/typing/re/dataclasses/enum`:
       - [x] 1モジュールずつ compile-run 可否を確認し、失敗要因を個別タスクへ分解する。
       - [x] 各モジュールが通るたびに `*_extended.py` を runtime 回帰へ昇格する。: `argparse/sys/typing/re/dataclasses/enum/json` を `tools/unittest/test_py2cpp_features.py` へ追加済み
  - [x] 接続後に `*_extended.py` を compile-run 回帰へ昇格する（`stdlib` 10/10 compile-run は達成済み、unit 回帰追加が未完）。: `python3 tools/unittest/test_py2cpp_features.py` 63 tests pass
   ...
   - [x] `sample/py/01_mandelbrot.py` を selfhost 経路で `-o` 生成できるところまで回復する。
     ...
     - [x] pure selfhost（中間 Python 呼び出しなし）で `.py -> -o` を通す。
      - [x] `selfhost/py2cpp.out` 側に `load_east(.py)` の実処理を実装し、`not_implemented` を返さないようにする。
         ...
         - [x] selfhost 最小モードの `load_east(.json)` を復活させ、`tools/check/check_selfhost_cpp_diff.py --selfhost-driver bridge` を再び通す（`std::any` 退化を回避した実装にする）。
           - [x] `.json` 読み込み自体は復活し、`bridge-json-unavailable` skip は解消した（2026-02-20）。
           - [x] `tools/check/check_selfhost_cpp_diff.py --selfhost-driver bridge` の既定ケースで `mismatches=0` を達成する。: `test/fixtures/core/add.py`, `test/fixtures/control/if_else.py`, `test/fixtures/collections/comprehension_filter.py`, `sample/py/01_mandelbrot.py` で `mismatches=0` を確認済み（2026-02-20）。
...
3. [x] CodeEmitter hook 移管の再開（selfhostを壊さない手順）
   ...
   - [x] `cpp_hooks.py` は最初に `png/gif` のみ移管し、`py2cpp.py` の既存分岐を1件ずつ削る。: `src/py2cpp.py` から `png/gif` 固有分岐が消えており、`src/hooks/cpp/hooks/cpp_hooks.py` 側へ集約済み（2026-02-21）。
    ...
    - [x] 置換ごとに `tools/check/check_py2cpp_transpile.py` を実行し、差分を確認する。: 2026-02-20 時点で `checked=119 ok=119 fail=0 skipped=5` を確認。
   - [x] 各ステップで `tools/build_selfhost.py` と `tools/check/check_py2cpp_transpile.py` の両方を必須ゲートにする。
...
1. [x] `src/py2cpp.py` の未移行ロジックを `CodeEmitter` 側へ移し、行数を段階的に削減する。
   ...
   - [x] ベースライン行数を計測し、`docs/ja/todo/index.md` に記録する（毎回更新）。: 2026-02-19 時点 `src/py2cpp.py` は `4305` 行（`wc -l`）
...
1. [x] `CodeEmitter` の `Any/dict` 境界を selfhost で崩れない実装へ段階移行する。
   ...
   - [x] `render_cond` / `get_expr_type` / `_is_redundant_super_init_call` で `optional<dict>` 混入をなくす。
     ...
     - [x] `render_cond` の同等整理（`optional<dict>` 境界の削減）を実施した。: `repr` の動的値フォールバックと外側括弧除去を追加し、`tools/unittest/test_code_emitter.py` で回帰固定（2026-02-20）。
   ...
   - [x] `tools/unittest/test_code_emitter.py` に selfhost 境界ケース（`None`, `dict`, `list`, `str`）を追加する。
   ...
   - [x] 回帰防止として `tools/unittest/test_py2cpp_codegen_issues.py` に「`get(\"kind\")` 直参照禁止」テストを追加した（2026-02-20）。
...
2. [x] `cpp_type` と式レンダリングで `object` 退避を最小化する。
   - [x] `str|None`, `dict|None`, `list|None` の Union 処理を見直し、`std::optional<T>` 優先にする。: 重複 Union（例: `list[int64]|None|None`）も `optional<...>` に正規化するよう更新し、`tools/unittest/test_cpp_type.py` を追加（2026-02-20）。
   - [x] `Any -> object` が必要な経路と不要な経路を分離し、`make_object(...)` の過剰挿入を減らす。
     - [x] `render_expr_as_any` を `_box_expr_for_any` 経由へ統一し、`dict[str, object]` の `AnnAssign` 初期化で既に `object` 型の値（例: `x: object`）を再 boxing しないよう修正した。: `tools/unittest/test_py2cpp_codegen_issues.py` に回帰テストを追加（2026-02-22）。
   ...
   - [x] `py_dict_get_default` / `dict_get_node` の既定値引数が `object` 必須になる箇所を整理する。
     - [x] `dict[str, object] | None` の `dict.get` が `d.get(...)` 直呼びへ退化する経路を `cpp_hooks.py` で補正し、`int`/`bool` 既定値は `dict_get_int` / `dict_get_bool` wrapper へ寄せた。: `tools/unittest/test_py2cpp_codegen_issues.py` に回帰テストを追加（2026-02-22）。
   ...
   - [x] `py2cpp.py` で `nullopt` を default 値に渡している箇所を洗い出し、型ごとの既定値へ置換する。
     - [x] `dict[str, V].get(key, None)`（`V` が optional でない場合）で `::std::nullopt` を直接渡さず、`V()` へ変換するよう `cpp_hooks.py` を修正した。: `tools/unittest/test_py2cpp_codegen_issues.py` に回帰テストを追加（2026-02-22）。
...
3. [x] selfhost コンパイルエラーを段階的にゼロ化する。
   ...
   - [x] 段階ゲート A: `total_errors <= 450` を達成し、`docs/ja/todo/index.md` に主因更新。: 2026-02-18 再計測で `total_errors=322`
   - [x] 段階ゲート B: `total_errors <= 300` を達成し、同時に `test_code_emitter` 回帰を固定化。: 2026-02-18 再計測で `total_errors=233`、`tools/unittest/test_code_emitter.py` は 12/12 pass
   - [x] 段階ゲート C-1: `total_errors <= 100` を維持する。: 2026-02-19 再計測で `total_errors=72`
   - [x] 段階ゲート C-2: `tools/check/check_selfhost_cpp_diff.py` の最小ケースを通す。: `--mode allow-not-implemented` で `mismatches=0`
...
5. [x] selfhost 版と Python 版の変換結果一致検証を自動化する。
   ...
   - [x] `selfhost/py2cpp.out` と `python3 src/py2cpp.py` の出力差分チェックをスクリプト化する。: `tools/check/check_selfhost_cpp_diff.py`
   - [x] CI 相当手順（ローカル）に組み込む。: `tools/run/run_local_ci.py`
...
- 更新（2026-02-19 selfhost 進捗）:
  ...
  6. `tools/check/check_selfhost_cpp_diff.py` に `--mode allow-not-implemented`（既定）を追加し、現段階の selfhost 未実装ケースを skip 集計できるようにした。
  7. `py_runtime.h` の `optional<dict<...>>` 向け `py_dict_get` が temporary 参照返却警告を出していたため、値返しへ変更して selfhost ビルド警告を解消。
  8. `tools/run/run_local_ci.py` を追加し、`check_py2cpp_transpile` / unit tests / selfhost build / selfhost diff（allow-not-implemented）を1コマンドで実行できるようにした。
  ...
  15. `tools/check/check_selfhost_cpp_diff.py --selfhost-driver bridge` の `sample/py/01_mandelbrot.py` 差分を解消（引数順崩れ/`png.write_rgb_png` 残留/浮動小数リテラル差分を修正）。
...
- 更新（2026-02-21 selfhost 進捗）:
  ...
  2. `tools/check/check_selfhost_cpp_diff.py --selfhost-driver bridge` は既定ケースで `mismatches=0` を再確認。
  ...
  17. `tools/check/check_selfhost_cpp_diff.py --selfhost-driver bridge` の既定ケースで `mismatches=0` を確認。
