...
2. [x] [ID: P20-CPP-SELFHOST-S6] `run_selfhost_parity.py --selfhost-lang cpp --emit-target cpp --case-root fixture` で fixture parity が PASS することを確認する
   ...
   - 2026-04-27: C++ ユーザーモジュールヘッダを `__pytra_user/` 配下へ逃がして `string.h` など標準ヘッダとの self-shadowing を回避し、C++ header generator の varargs / trait implements / pure virtual trait method 宣言を selfhost 生成 `.cpp` と一致させた。さらに selfhost parity runner を `eo_` emit-only fixture 対応に揃え、`python3 tools/run/run_selfhost_parity.py --selfhost-lang cpp --emit-target cpp --case-root fixture` が `fixture_pass=161 / fixture_fail=0` で PASS。
3. [x] [ID: P20-CPP-SELFHOST-S7] `run_selfhost_parity.py --selfhost-lang cpp --emit-target cpp --case-root sample` で sample parity が PASS することを確認する
   - 2026-04-27: 古い runtime EAST に残る `with open(...) as f` lowering でも C++ emitter が `__enter__()` 代入を参照束縛として出すよう防御し、GIF 系 sample の `PyFile` copy compile error を修正。selfhost C++ バイナリを再生成・link し、`python3 tools/run/run_selfhost_parity.py --selfhost-lang cpp --emit-target cpp --case-root sample` が `sample_pass=18 / sample_fail=0` で PASS。
