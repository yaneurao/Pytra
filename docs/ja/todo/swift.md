...
1. [ ] [ID: P1-EMITTER-SELFHOST-SWIFT-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/swift/cli.py --target cpp -o work/selfhost/emit/swift/` を実行し、変換が通るようにする
   - 進捗: 2026-04-29 に実行し、変換は未 PASS。`rm -rf work/selfhost/emit/swift && timeout 180s python3 src/pytra-cli.py -build src/toolchain/emit/swift/cli.py --target cpp -o work/selfhost/emit/swift/` は `error: build failed: expected ) but got "var __out: [Any] = []; "` で失敗する。
