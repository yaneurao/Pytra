...
1. [ ] [ID: P1-HOST-CPP-EMITTER-LUA-S1] `python3 src/pytra-cli.py -build src/toolchain/emit/cpp/cli.py --target lua -o work/selfhost/host-cpp/lua/` で変換 + build を通す
   ...
   - 進捗: 2026-04-30 に `src/toolchain/emit/lua/cli.py` へ runtime copy を追加し、`src/toolchain/emit/lua/emitter.py` の entry `__script_dir` global 化、table literal 直接 index の括弧付け、`field(default_factory=...)` lowering、type-object subscript alias skip を実装した。`src/runtime/lua/built_in/py_runtime.lua` は callable `Path` table、Python 互換 `sys.argv`、`json.JsonValue`/`JsonObj`/`JsonArr`、`loads_obj`/`loads_arr`、`__pytra_dict_ctor` を追加済み。実行は manifest 32 modules の読み込みと C++ emitter 本体まで進み、`toolchain_emit_cpp_emitter.lua` の `_build_class_symbol_fqcn_map` で `raw_obj` nil により停止する。
