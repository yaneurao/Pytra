...
3. [ ] [ID: P1-EMITTER-SELFHOST-ZIG-S3] コンパイル済み emitter で既存 fixture の manifest を処理し、Python 版 emitter と parity 一致を確認する
   - 進捗: 2026-05-01 に S2 で生成した `work/selfhost/emit/zig/emitter_zig_cpp` を `tools/run/run_selfhost_parity.py --selfhost-lang cpp --selfhost-bin work/selfhost/emit/zig/emitter_zig_cpp --emit-target zig --case-root fixture --category core` で検証開始。core 22 件は selfhost transpile 段階で fail（runner 上は stderr 空）し、`add` の direct invoke では当初 exit 139。ASan で `_dict_list(self._json_to_any(east_doc["body"]))` と statement `leading_trivia` の `list[Any]`/`list[JsonVal]` 読み違いを確認したため、root EAST list は `_json_dict_list` 経由、statement leading trivia は一旦 no-op に変更。`python3 -m py_compile` / `git diff --check` / `timeout 3600s python3 src/pytra-cli.py -build ...` / `timeout 3600s g++ -std=c++20 -O0 ... -o work/selfhost/emit/zig/emitter_zig_cpp` は PASS 継続。`add` direct invoke は exit 134 `std::bad_variant_access: std::visit: variant is valueless` まで進んだため、次回はこの runtime abort のスタック特定から継続する。
