...
- [ ] [ID: P0-PYTRA-NES2-CROSSBACKEND-REPRO-01] Pytra-NES2 repro を全 backend representative contract に昇格する。進捗: `S1-02` で `property_method_call` と `list_bool_index` を `test/fixtures/typing/` へ昇格し、期待 semantics を assertion 付きで固定済み。次は全 backend smoke にこの fixture を接続する。文脈: [p0-pytra-nes2-crossbackend-repro-contract.md](../plans/p0-pytra-nes2-crossbackend-repro-contract.md)
