...
> **2026-04-07** — lint 149 件 / 14 言語 10/10 PASS（697→149）。PyFile 廃止、IOBase 型階層を `built_in/io.py` に定義。emitter guide §12.7 追加。全言語横断で PyFile coupling 除去。Go/Rust/C#/PHP/Ruby/Lua emitter guide 整合。

> **2026-04-06** — with 文を `__enter__`/`__exit__` プロトコルで実装（CommonRenderer try/finally + hoist）。with fixture 2 件追加。parity FAIL 時 work dir 保持。TS/JS shim cleanup 完了。Dart emitter guide 準拠。JVM backend 大幅進展（Scala/Kotlin emitter 新規実装）。.east* 生成物を git 管理から除去。

> **2026-04-05** — containers.py `mut[T]` 注釈で `meta.mutates_receiver` 導入。C++ メソッド名ハードコード解消。mapping.json FQCN キー統一完了。toolchain リネーム完了。

> **2026-04-04** — emitter lint に Python メソッド名ハードコード検出追加（23 パターン）。parity check の全件 SKIP → FAIL 修正。callable_optional_none fixture 追加。emitter guide §12.6 callable 型写像・§14.1 lint ドキュメント追加。
