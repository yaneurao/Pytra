...
> **2026-04-03** — emitter 呼び出し構造を統一（共通ランナー `cli_runner.py` で 17 言語対応）。`resolved_type: "object"` を EAST3 validator で全面禁止。IsInstance から `PYTRA_TID_*` 廃止し `expected_type_name` に移行。`--east3-opt-level` を `--opt-level` に改名。`@extern class` opaque 型 spec 確定。`builtin_name` 廃止。runtime_call カバレッジ lint 新設。全件 SKIP を FAIL に変更。`tools/unregistered/` 削除。Julia / Zig / PowerShell / Swift / Dart backend TODO 新設。

> **2026-04-02** — C++ monostate → `optional<variant>` 移行。bounds check / negative index を EAST optimizer に移管（sample 01 mandelbrot 12.8s → 0.82s）。png.py / gif.py の extend 最適化（全言語高速化）。emitter guide §12.4-12.6 新設（Optional/union/callable 型写像）。fixture リネーム（any_* → union_*/optional_none）。

> **2026-04-01** — emitter hardcode lint runtime キャッシュ機構。正本ファイル変更禁止ルール。EAST3 copy elision メタデータ計画起票。C++ prefix_match lint 修正。
