...
| `@dataclass` | supported | サポート済み。representative static subset として `field(default=...)`、`field(default_factory=deque)`、`field(default_factory=Child)` on `rc<Child>` まで compile smoke で固定している。arbitrary callable `default_factory` や Python dataclasses 完全互換は対象外。 | `test/unit/test_py2cpp_features.py:1519`, `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/ir/test_east_core_parser_behavior_classes.py`, `test/fixtures/stdlib/dataclasses_extended.py:1` |
| `collections.deque[T]`（representative lane） | partial | `deque[T]` の C++ type lowering は `::std::deque<T>` に固定済みで、`@dataclass` field の `init=False` zero-arg ctor / `default_factory=deque` に加えて、representative expression / method subset として `copy()` / `index(value)` まで build/run smoke で確認済み。full `deque` API 互換、`index()` の slice overload、plain local inference (`r = q.copy()`) はまだ対象外。 | `src/backends/cpp/emitter/type_bridge.py`, `src/backends/cpp/emitter/header_builder.py`, `src/runtime/cpp/native/core/py_types.h`, `test/unit/backends/cpp/test_py2cpp_features.py` |
| `str(Path(...))`（representative lane） | supported | `Path` の stringify は representative C++ lane で `path.__str__()` に lower され、generic `py_to_string(T)` fallback へは戻らない。compile/run smoke で固定済み。 | `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/backends/cpp/test_cpp_runtime_iterable.py`, `test/fixtures/stdlib/path_stringify.py:1` |
