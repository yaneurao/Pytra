...
| `collections.deque[T]`（representative lane） | partial | `deque[T]` の C++ type lowering は `::std::deque<T>` に固定済みで、`@dataclass` field の `init=False` zero-arg ctor と `default_factory=deque` までは compile smoke で確認済み。full `deque` API 互換や plain class の general ctor policy はまだ対象外。 | `src/backends/cpp/emitter/type_bridge.py`, `src/backends/cpp/emitter/header_builder.py`, `src/runtime/cpp/native/core/py_types.h`, `test/unit/backends/cpp/test_py2cpp_features.py` |
