...
| `@dataclass` | supported | サポート済み。representative static subset として `field(default=...)`、`field(default_factory=deque)`、`field(default_factory=Child)` on `rc<Child>` に加えて、Pytra-NES representative package の `timestamps: deque[float] = field(init=False, repr=False)` multi-file build/run smoke まで固定している。arbitrary callable `default_factory` や Python dataclasses 完全互換は対象外。 | `test/unit/test_py2cpp_features.py:1519`, `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/ir/test_east_core_parser_behavior_classes.py`, `test/fixtures/stdlib/dataclasses_extended.py:1` |
