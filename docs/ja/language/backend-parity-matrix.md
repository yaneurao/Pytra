...
- native backend の wildcard relative import は [20260314-p0-relative-wildcard-import-native-rollout.md](../plans/archive/20260314-p0-relative-wildcard-import-native-rollout.md) で close し、`go/java/kotlin/lua/nim/php/ruby/scala/swift` は representative module-graph bundle で `transpile_smoke_locked + module_graph_bundle_transpile` に固定した。single-file direct lane は全 backend で `backend_specific_fail_closed` のまま維持する。
