...
- next rollout handoff は [p1-relative-import-native-path-bundle.md](../plans/p1-relative-import-native-path-bundle.md) を参照し、`rs/cs/js/ts` を `transpile_smoke_locked` baseline に固定したうえで、残る non-C++ lane は `backend_specific_fail_closed` を維持しながら、`locked_js_ts_smoke_bundle -> native_path_bundle(go/nim/swift) -> jvm_package_bundle(java/kotlin/scala)` の順で進める。`go/nim/swift` は `native_path_bundle_rollout`、`java/kotlin/scala` は `remaining_second_wave_rollout_planning`、`lua/php/ruby` は `defer_until_second_wave_remaining_complete` のまま維持する。
