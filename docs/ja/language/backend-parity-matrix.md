...
- next rollout handoff は [p1-relative-import-native-path-bundle.md](../plans/p1-relative-import-native-path-bundle.md) を参照し、bundle order は `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle` に固定する。`rs/cs/js/ts` を `transpile_smoke_locked` baseline に固定したうえで、`go/nim/swift` は representative smoke を lock しつつ active `native_path_bundle_rollout` として残す。follow-up の `java/kotlin/scala` は `jvm_package_bundle` として `remaining_second_wave_rollout_planning` に維持し、`lua/php/ruby` は `defer_until_second_wave_remaining_complete` のまま据え置く。current non-C++ fail-closed lane は明示的に `backend_specific_fail_closed` を使う。
