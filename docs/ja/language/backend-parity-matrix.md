...
- 現時点で `build_run_locked` なのは `cpp`、`transpile_smoke_locked` なのは `rs/cs/go/java/js/kotlin/lua/nim/scala/swift/ts` です。`go/nim/swift` は direct native emitter の function-body transpile smoke、`java/kotlin/scala` は JVM package bundle の `package_project_transpile`、`lua` は representative relative import alias rewrite の `native_emitter_function_body_transpile` を証跡 lane に固定しています。`php/ruby` は support claim としてはまだ未主張で、archived long-tail fail-closed bundle による `fail_closed_locked` と `backend_native_fail_closed` の baseline を維持しています。
...
- next rollout handoff は [p1-relative-import-longtail-support.md](../plans/p1-relative-import-longtail-support.md) を参照し、historical bundle order は `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle` のまま固定する。`java/kotlin/scala` は archived JVM bundle として `transpile_smoke_locked` baseline に移り、active `longtail_relative_import_support_rollout` は current bundle を `mixed_rollout_locked + mixed_backend_evidence` として扱う。`lua` は `native_emitter_function_body_transpile` を current evidence lane にした `transpile_smoke_locked` representative lane、`php/ruby` は `backend_specific_fail_closed` / `backend_native_fail_closed` の residual fail-closed lane として残る。backend-local focused smoke lane は `lua_relative_import_support_rollout_smoke` / `php_relative_import_support_rollout_smoke` / `ruby_relative_import_support_rollout_smoke` に固定し、current bundle の smoke-locked backend は `lua`、remaining rollout backends は `php/ruby`、followup rollout bundle / followup verification lane はどちらも `none` とする。
