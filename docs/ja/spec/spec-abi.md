...
- `value`
  - 戻り値または immutable/value-like 引数を、ABI 正規形の value ABI として固定する
- `value_readonly`
  - 引数を value ABI 正規形として固定する
  - callee はこの引数を破壊的に変更してはならない

補足:

- `value_readonly` は特に C++ の `list/dict/set/bytearray` に対して重要である。
...
`@abi` の `value` / `value_readonly` は、この ABI 正規形を helper 境界へ明示的に適用するための注釈である。
...
### 6.4 `@abi(value_readonly)` の追加ルール

`@abi(args={"x": "value_readonly"})` の引数 `x` については、backend は次を守る。
