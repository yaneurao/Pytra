...
- `src/pytra-cli.py` / `tools/gen/gen_makefile_from_manifest.py` / `./pytra` は実装済みで、`--target cpp --build` の導線を `./pytra` 経由で提供します。
...
4. `tools/gen/gen_makefile_from_manifest.py` で `Makefile` を生成する。
...
`tools/gen/gen_makefile_from_manifest.py` を使用し、次を受け付ける。
...
- `python3 tools/check/runtime_parity_check.py --targets cpp --case-root sample --all-samples --cpp-codegen-opt 3 --east3-opt-level 2` が green を維持する。
...
1. Phase 1: `tools/gen/gen_makefile_from_manifest.py` を追加する。
...
- `tools/check/runtime_parity_check.py` など周辺ツールで target 固有 build/run コマンドを重複定義してはならない（`pytra-cli` 経由に統一）。
