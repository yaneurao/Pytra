...
- `tools/check/check_py2java_transpile.py` が native 経路で通る。
- `tools/unittest/test_py2java_*.py` で native-only 前提アサーションを固定する。
- `tools/check/runtime_parity_check.py --targets java` で Python 基準との出力一致を監視する。
