...
- 未移行 target では `.gen/.ext` を継続してよい。C++ `core` については承認済み次段で plain naming (`core/*.h`, `native/core/*.{h,cpp}`) へ移行する。
...
- `src/runtime/cpp/core/py_runtime.h`（承認済み次段の plain naming）
...
### 0.601 C++ core runtime の承認済み ownership split

`src/runtime/cpp/core/` は low-level runtime の責務名として維持するが、ownership については次段で次の split を導入する。
...
- 承認済み次段では `core` surface は `*.h`、`native/core` 正本は `*.h` / `*.cpp` の plain naming に揃え、`.ext` は移行中 legacy 名としてのみ扱う。
- `pytra/core` は導入しない。
...
- core file naming は承認済み次段で plain name を正本とし、`.ext` suffix を再導入してはならない。
