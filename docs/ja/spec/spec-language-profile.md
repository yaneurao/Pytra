...
- `src/backends/<lang>/profiles/`
  - `profile.json`: 言語向け統合プロファイル（エントリ）
  - `types.json`: 型マップ
  - `operators.json`: 演算子マップ
  - `runtime_calls.json`: 組み込み・`module.attr` 呼び出しマップ
  - `syntax.json`: 文テンプレート

## 3. ロード順序

1. `src/backends/common/profiles/core.json`
2. `src/backends/<lang>/profiles/profile.json` の `include` 順
3. `src/backends/<lang>/profiles/profile.json` 本体
4. CLI 上書き（必要時）
...
`module` は言語固有側（例: `src/backends/cpp/emitter/`）に配置し、`src/backends/common/` へは置きません。
