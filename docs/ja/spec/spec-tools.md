...
- `tools/check_runtime_cpp_layout.py`
  - 目的: `src/runtime/cpp/{built_in,std,utils}` の legacy-closed 維持、`generated/native/pytra` の ownership 境界、`core` compatibility surface と `generated/core` / `native/core` の split 前提を同一 guard で検証する。あわせて `generated/core` / `native/core` lane の存在を要求し、`runtime/cpp/native/core/...` の直接 include を `core/*.ext.h` forwarder 以外で禁止する。
