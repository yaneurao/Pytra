...
- 共通 smoke（CLI 成功、`--east-stage 2` 拒否、`load_east`、add fixture）は `test/unit/test_py2x_smoke_common.py` を正本とする。
- 言語別 smoke（`test/unit/test_py2*_smoke.py`）は言語固有の emitter/runtime 契約だけを保持し、共通ケースを再実装しない。
