...
- 2026-03-14: Added another `S2-01` bundle to rename the remaining `std/math` postprocess hooks from `rs_std_math_live_wrapper` / `php_std_math_live_wrapper` to `rs_math_runtime_wrapper` / `php_math_runtime_wrapper`, synchronizing the manifest, generator, inventory, and unit tests around generic math-runtime seam names.
