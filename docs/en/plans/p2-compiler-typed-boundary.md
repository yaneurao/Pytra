...
- 2026-03-10 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02]: The next slice added `_sh_make_expr_token()` and `_sh_make_import_binding()` in `src/toolchain/ir/core.py`, then moved `ExprParser._tokenize()` and `_sh_append_import_binding()` onto helper-owned carriers. The tracked generated mirror in `selfhost/runtime/cpp/pytra-gen/compiler/east_parts/core.cpp` gained matching helpers too, so tokenizer/import-binding paths no longer repeat open-coded inline `make_object` dicts; `test_prepare_selfhost_source.py` now guards the new helper boundary against regressions.
