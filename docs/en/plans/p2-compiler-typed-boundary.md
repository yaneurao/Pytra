...
- 2026-03-09 [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-01]: A further slice added `_sh_make_constant_expr()` and rewired `Name` / `Tuple` / `Constant` creation in `_parse_comp_target()`, `_parse_primary()`, and the simple list-comp/tuple lowering helpers through the new builder seam. That narrows the remaining large raw-assembly clusters mostly to the broader expression envelope and the `Subscript` / `Attribute` / `Call` families.
