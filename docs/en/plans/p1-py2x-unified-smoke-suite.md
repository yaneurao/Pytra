...
Background:
- `tools/unittest/test_py2*_smoke.py` duplicates common verification viewpoints across languages (CLI path, `--east-stage 2` rejection, minimal fixture conversion).
...
In scope:
- Inventory common smoke viewpoints scattered across `tools/unittest/test_py2*_smoke.py`
...
- Confirm consistency with `tools/check/check_py2*_transpile.py` and existing unit execution paths
...
- Target: 14 files in `tools/unittest/test_py2*_smoke.py` (`cpp,rs,cs,js,ts,go,java,swift,kotlin,rb,lua,scala,php,nim`)
