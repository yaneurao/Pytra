...
- 2026-03-13: `S2-02` restored reproducible Wave A live regeneration on the current tree. Nim now falls back to the checked-in helper artifact shape for `Try` (`# unsupported stmt: Try`), which lets `utils/png.py` and `utils/gif.py` regenerate their helper lanes again. Java also gained the missing string-literal escapes, which removes the stale `generated/std/json.java` compare lane. After those fixes, `gen_runtime_from_manifest.py --targets go,java,kotlin,scala,swift,nim --check` passes.
