...
- 2026-03-13: As the third `S1-02` bundle, the logical module buckets derived from the target inventory (`generated/native/compat`) and the per-backend blocked-module baseline were also fixed in the contract. The checker allows compat overlap with native/generated, but requires blocked modules to stay disjoint from every live bucket.
