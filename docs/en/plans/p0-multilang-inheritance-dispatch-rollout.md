<a href="../../ja/plans/p0-multilang-inheritance-dispatch-rollout.md">
...
- verification: `tools/check/runtime_parity_check.py` / `tools/unittest/test_py2*_smoke.py`
...
- 2026-03-01: Added `inheritance_virtual_dispatch_multilang` to default fixture cases in `tools/check/runtime_parity_check.py` and connected it to the regression path.
...
- 2026-03-01: Ran `python3 tools/check/runtime_parity_check.py inheritance_virtual_dispatch_multilang --targets cpp,rs,cs,js,ts,go,java,swift,kotlin,ruby,lua --ignore-unstable-stdout --summary-json out/inherit_dispatch_multilang_summary.json` and pinned a pre-S2 implementation baseline (`run_failed=10`, `toolchain_missing=1`).
