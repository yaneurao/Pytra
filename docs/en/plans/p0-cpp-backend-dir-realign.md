...
Background:
- Under `src/backends/cpp/`, the folders `hooks/`, `header/`, `multifile/`, `profile/`, and `runtime_emit/` still remain and do not match the target responsibility boundary (`lower/optimizer/emitter`).
...
Goal:
- Realign implementation boundaries in `src/backends/cpp/` around `lower/optimizer/emitter`, and remove helper folders from the backend root.
...
- `src/backends/cpp/emitter/*` (new destination modules)
...
Acceptance criteria:
- `hooks/`, `header/`, `multifile/`, `profile/`, and `runtime_emit/` no longer exist directly under `src/backends/cpp/`.
- Legacy imports (`backends.cpp.hooks|header|multifile|profile|runtime_emit`) are removed from production code.
...
- [x] [ID: P0-CPP-DIR-REALIGN-01-S2-06] Remove the legacy five folders and unify `backends.cpp.*` imports to new paths.
...
| `backends/cpp/hooks/` | C++ emitter hook registry (`build_cpp_hooks`) | `backends/cpp/profile/cpp_profile.py`, `src/py2cpp.py`, tests | `backends/cpp/emitter/hooks_registry.py` |
| `backends/cpp/runtime_emit/` | Runtime path / include / namespace resolution | `src/py2cpp.py`, `backends/cpp/emitter/module.py` | `backends/cpp/emitter/runtime_paths.py` |
| `backends/cpp/header/` | EAST -> C++ header generation | `src/py2cpp.py` | `backends/cpp/emitter/header_builder.py` |
| `backends/cpp/multifile/` | Multi-file output orchestration | `src/py2cpp.py` | `backends/cpp/emitter/multifile_writer.py` |
...
- Canonical directories directly under `src/backends/cpp/` are limited to `lower/`, `optimizer/`, and `emitter/`.
- Helper modules are placed directly under `emitter/` with these names:
  ...
  - `header_builder.py`
  - `multifile_writer.py`
- Legacy imports (`backends.cpp.{profile,hooks,runtime_emit,header,multifile}`) are disallowed after phased migration.
- `py2cpp.py` imports backend helpers only from `backends.cpp.emitter.*`.
- Backward-compat aliases are allowed only during migration and are removed upon completion of `S2-06`.

Decision log:
- 2026-03-02: Per user direction, filed cleanup of the five root-level folders in `src/backends/cpp/` (`hooks/header/multifile/profile/runtime_emit`) as P0.
...
- 2026-03-02: [ID: P0-CPP-DIR-REALIGN-01-S1-02] Documented naming conventions and import boundaries (unify to `backends.cpp.emitter.*`).
- 2026-03-02: [ID: P0-CPP-DIR-REALIGN-01-S2-01] Moved `src/backends/cpp/profile/cpp_profile.py` to `src/backends/cpp/emitter/profile_loader.py`, updated imports in `py2cpp`/`CppEmitter`/related helpers, and validated no regressions with `check_py2cpp_transpile.py` and `test_language_profile.py`.
- 2026-03-02: [ID: P0-CPP-DIR-REALIGN-01-S2-02] Moved `src/backends/cpp/hooks/cpp_hooks.py` to `src/backends/cpp/emitter/hooks_registry.py`, updated hook-factory references in `py2cpp`, profile loader, and `profiles/cpp/profile.json`, and verified no regressions with `test_cpp_hooks.py` and `check_py2cpp_transpile.py`.
- 2026-03-02: [ID: P0-CPP-DIR-REALIGN-01-S2-03] Moved `src/backends/cpp/runtime_emit/cpp_runtime_emit.py` to `src/backends/cpp/emitter/runtime_paths.py`, updated runtime-path imports in `py2cpp`/`CppModuleEmitter`, and verified no regressions with `test_py2cpp_features.py -k runtime_module_tail_and_namespace_support_compiler_tree` and `check_py2cpp_transpile.py`.
- 2026-03-03: [ID: P0-CPP-DIR-REALIGN-01-S2-04] Moved `src/backends/cpp/header/cpp_header.py` to `src/backends/cpp/emitter/header_builder.py` and updated imports in `py2cpp`.
- 2026-03-03: [ID: P0-CPP-DIR-REALIGN-01-S2-05] Moved `src/backends/cpp/multifile/cpp_multifile.py` to `src/backends/cpp/emitter/multifile_writer.py` and updated imports in the multi-file output path.
- 2026-03-03: [ID: P0-CPP-DIR-REALIGN-01-S2-06] Removed Python implementations under `src/backends/cpp/{hooks,header,multifile,profile,runtime_emit}` and removed legacy import references.
