...
Background:
- When updating `sample/scala` from `sample/py`, currently `tools/gen/regenerate_samples.py` does not include `scala` in its language list.
...
Goal:
- Officially support `--langs scala` in `tools/gen/regenerate_samples.py` and integrate `sample/scala` regeneration into the existing path.
...
Acceptance criteria:
- `python3 tools/gen/regenerate_samples.py --langs scala --force` succeeds and can batch-regenerate `sample/scala/*.scala`.
...
- `python3 tools/gen/regenerate_samples.py --langs scala --dry-run --verbose`
...
- [x] [ID: P0-REGEN-SCALA-SUPPORT-01-S2-01] Add `scala` config to `tools/gen/regenerate_samples.py` so CLI accepts `--langs scala`.
...
- 2026-03-02: [ID: P0-REGEN-SCALA-SUPPORT-01-S2-01] Added Scala config in `tools/gen/regenerate_samples.py` (`cli=src/py2scala.py`, `out_dir=sample/scala`, `ext=.scala`) and to `--langs` defaults, resolving `unknown language(s): scala`.
...
- 2026-03-02: [ID: P0-REGEN-SCALA-SUPPORT-01-S3-01] Ran `python3 tools/gen/regenerate_samples.py --langs scala --force` and confirmed `summary: total=18 skip=0 regen=18 fail=0`.
