...
Breakdown:
- [x] [ID: P4-CROSSRUNTIME-PYRUNTIME-FINAL-THINCOMPAT-REMOVAL-01] Remove the final `py_runtime.h` thin compat helpers across runtimes.
...
- [x] [ID: P4-CROSSRUNTIME-PYRUNTIME-FINAL-THINCOMPAT-REMOVAL-01-S3-01] Remove template `py_runtime_type_id` / `py_isinstance` from `py_runtime.h` and refresh representative regressions/docs/archive.
...
- 2026-03-11: Completed `S3-01` by removing template `py_runtime_type_id` / `py_isinstance` from `py_runtime.h` and moving the representative C++ runtime/codegen/header-surface regressions onto the thin helper surface (`py_runtime_object_isinstance`). The `shared_type_id_compat` bucket is now intentionally empty, so the final header thin-compat task can be archived.
