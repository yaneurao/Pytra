...
- [x] [ID: P1-NONCPP-RUNTIME-LAYOUT-ROLLOUT-REMAINING-01-S4-01] Update cross-backend guards / inventories / docs to the `generated/native` vocabulary so no backend remains incomparable.
...
- 2026-03-13: In the following `S3-03` bundle, `php` `std/json` was promoted into a live-generated compare lane from the SoT. To unblock the generator, the PHP emitter now lowers `RuntimeIter` tuple targets through a key/value `foreach` path for `dict.items()`, `tools/gen/gen_runtime_from_manifest.py` plus `tools/runtime_generation_manifest.json` gained a PHP `std/json` live-wrapper postprocess, `src/runtime/php/generated/std/json.php` was regenerated, and PHP runtime packaging was extended into the generated lane. The contract/checker and tooling/smoke coverage were synchronized, which shrinks the PHP Wave B blocked compare baseline down to `built_in/*` only.
...
- 2026-03-13: The current `S4-01` bundle also adds the `kotlin/scala/nim` generated `built_in` compare baseline to the manifest and contract. `kotlin/scala` materialize `contains/iter_ops/predicates/sequence/zip_ops`, while `nim` materializes `contains/iter_ops/numeric_ops/predicates/zip_ops`, all under `generated/built_in/*` as the compile-safe subset. `tools/gen/gen_runtime_from_manifest.py` gained Kotlin/Scala `program_to_library` postprocess hooks that strip the empty generated `main`, and the checker plus tooling/backend smoke coverage now track `kotlin/nim = build_run_smoke` and `scala = source_guard`. That reduces the remaining compare-impossible Wave A backend set to `swift` only. `swift` stays blocked in this slice because its native substrate still lacks helper functions such as `__pytra_as_list` that the generated built_in lane expects.
- 2026-03-13: The current `S4-01` bundle also unblocks `swift`. `src/runtime/swift/native/built_in/py_runtime.swift` now provides `__pytra_as_list` and `__pytra_as_dict`, while `tools/gen/gen_runtime_from_manifest.py` gained a Swift `program_to_library` postprocess that strips the empty generated `@main`. That makes `contains/io_ops/iter_ops/predicates/sequence/zip_ops` a compile-safe generated subset under `src/runtime/swift/generated/built_in/*`, and the contract/checker plus Swift build-run smoke now track that live lane. The Wave A compare-impossible backend set is now empty.
- 2026-03-13: The `S4-01` close-out aligned the Wave A/Wave B generated compare baseline, smoke taxonomy, blocked/helper-shaped-gap inventory, and compat/native residual boundary on the `generated/native` vocabulary. With the compare-impossible set cleared, all remaining backends (`go/java/kotlin/scala/swift/nim/js/ts/lua/ruby/php`) are now file-level comparable in the cross-backend guards, inventories, and docs, so P1 is complete and moves to archive.
