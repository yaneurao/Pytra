...
- 2026-03-09 [ID: P1-EAST-TYPEEXPR-01-S6-02]: Expanded the reintroduction guard beyond the representative lane to cover `json.obj.get_int`, `json.arr.get_bool`, and `json.loads_obj`, locking `receiver_nominal_adt_name`, `result_type.nominal_adt_name`, and `decode_kind=module_load` in `tools/check_jsonvalue_typeexpr_contract.py`. The tool import path was normalized to `toolchain.*` so it stays stable both from repo-root execution and selfhost-adjacent module layouts. In parallel, `test_east2_to_east3_lowering.py` now runs the guard script directly, and `test_prepare_selfhost_source.py` now locks the selfhost seed's `_JSON_NOMINALS` / `NominalAdtType` helper path. Verified with `python3 tools/check_jsonvalue_typeexpr_contract.py`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/tooling -p 'test_check_jsonvalue_typeexpr_contract.py'`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/ir -p 'test_east2_to_east3_lowering.py' -k json`, `PYTHONPATH=src python3 -m unittest discover -s test/unit/selfhost -p 'test_prepare_selfhost_source.py'`, `python3 tools/build_selfhost.py`, `python3 tools/check_todo_priority.py`, and `git diff --check`.
