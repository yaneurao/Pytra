...
- `P0-ISINSTANCE-01-S4`: Reorganize tests (`tools/unittest/*isinstance*`) and TODO linkage.
...
- 2026-02-25: In `P0-ISINSTANCE-01`, Rust emitter was updated to runtime-API lowering via `py_isinstance`; helper output for `type_id` interval tables (`PyTypeInfo`) was added. Verified: `tools/unittest/test_py2rs_smoke.py` (22 cases), `tools/check/check_py2rs_transpile.py` (`checked=130 ok=130 fail=0 skipped=6`), `tools/check/check_py2cpp_transpile.py` (`checked=131 ok=131 fail=0 skipped=6`), `tools/check/check_py2cs_transpile.py` (`checked=130 ok=130 fail=0 skipped=6`). For JS/TS, existing `east3-contract` failures were bypassed with `--skip-east3-contract-tests` and verified at `checked=130 ok=130 fail=0 skipped=6`.
- 2026-02-25: As `P0-ISINSTANCE-01-S4`, resolved blocking failures in `test_east3_cpp_bridge.py` (kept strict EAST3 while allowing limited stage2/self_hosted compatibility path; re-unified `dict[str,*]` key conversion to `py_to_string`; re-fixed `render_cond(Any)` to `py_to_bool`). Verified normal-mode checks: `tools/check/check_py2cpp_transpile.py` (`checked=131 ok=131 fail=0 skipped=6`) and `tools/check/check_py2rs_transpile.py` / `tools/check/check_py2cs_transpile.py` / `tools/check/check_py2js_transpile.py` / `tools/check/check_py2ts_transpile.py` (each `checked=130 ok=130 fail=0 skipped=6`); plus `tools/unittest/test_py2{js,ts,cs,rs}_smoke.py`, `tools/unittest/test_js_ts_runtime_dispatch.py`, `tools/unittest/test_pytra_built_in_type_id.py`, and five `isinstance` cases in `test_py2cpp_codegen_issues.py` all green.
