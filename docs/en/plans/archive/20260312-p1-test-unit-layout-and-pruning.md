<a href="../../../ja/plans/archive/20260312-p1-test-unit-layout-and-pruning.md">
...
Background:
- `tools/unittest/` mixes language-specific tests, IR tests, tooling tests, and selfhost-related tests, which hurts discoverability and maintainability.
...
Scope:
- Re-layout files under `tools/unittest/`, for example into `common`, `backends/<lang>`, `ir`, `tooling`, and `selfhost`
...
- Main execution paths, including `unittest discover`, `tools/check/check_py2*_transpile.py`, and selfhost checks, work with the new paths.
...
- `rg -n "tools/unittest/|test_py2.*smoke" tools docs/ja docs/en -g '*.py' -g '*.md'`
...
- 2026-03-04: Completed `S2-01`. Moved 71 `tools/unittest/test_*.py` files into `common/backends/<lang>/ir/tooling/selfhost` via `git mv` and updated the fixed test paths in `tools/run/run_local_ci.py` and `tools/check/check_noncpp_east3_contract.py`. Because turning `tools/unittest/backends` into a package would conflict with `src/backends`, the policy is to avoid `__init__.py` there, with discover-path handling updated in `S2-02`.
- 2026-03-04: Completed `S2-02`. Replaced `tools/unittest/test_discovery_router.py` with a manual `load_tests + importlib` loader so `python3 -m unittest discover -s test/unit -p 'test*.py'` can recurse even under non-package directories. The previously broken `from comment_fidelity import ...` path was stabilized by adding root-level `comment_fidelity.py`, and the three smoke discovers for Go, Swift, and Kotlin were confirmed to pass. `tools/check/check_gsk_native_regression.py` was also updated to reference the new paths under `tools/unittest/emit/*`.
- 2026-03-04: Completed `S3-01`. Ran a reference scan over all `tools/unittest/test*.py` entries against `tools/docs` and extracted zero-reference candidates, `test_pylib_*` and `test_east3_to_human_repr.py`. All of them were judged to be active regressions under discover, so deletion and integration were both deferred and the result was `keep`.
...
- 2026-03-04: Completed `S4-01`. Re-ran the main regressions, including `tools/unittest/common/test_py2x_smoke_common.py`, `tools/unittest/emit/{go,swift,kotlin}/test_py2*_smoke.py`, `tools/unittest/selfhost/test_prepare_selfhost_source.py`, and `tools/check/check_noncpp_east3_contract.py --skip-transpile`, and confirmed all passed.
- 2026-03-04: Completed `S4-02`. Updated the test-path references in `docs/ja/spec` and `docs/en/spec`, previously pointing at `tools/unittest/test_*.py`, to the new `common/backends/ir/tooling/selfhost` layout.

## S3-01 Audit Memo (2026-03-04)

- Extraction procedure:
- Enumerate all `tools/unittest/test*.py` files and count `rg -n <basename> tools docs/ja docs/en` hits for each basename.
...
- `tools/unittest/common/test_pylib_argparse.py` and the other 8 `test_pylib_*` files:
- Decision: keep. `tools/unittest/common` discover passes 36 tests, and these files guard shim regressions under `src/pytra/*`, so they cannot be deleted.
- `tools/unittest/ir/test_east3_to_human_repr.py`:
- Decision: keep. A dedicated discover run under `tools/unittest/ir` passes 3 tests, and the file protects the compatibility-wrapper contract for `east2/east3` human-readable representations.
- `tools/unittest/test_discovery_router.py`:
- Decision: keep. It is required as the top-level discover router when `tools/unittest/backends` remains non-package.
- Audit conclusion:
...
- Total count: 71 files matching `tools/unittest/test*.py`
...
- `tools/unittest/emit/<lang>/`: `test_py2<lang>_smoke.py` and other backend-specific tests
- `tools/unittest/ir/`: `test_east*.py`
- `tools/unittest/tooling/`: CLI, manifest, parity-tool, and related operational tests
- `tools/unittest/selfhost/`: selfhost build, diff, and regression tests
- `tools/unittest/common/`: cross-language, pylib, profile, and bootstrap tests not covered above
...
- Root structure:
- `tools/unittest/common/`: shared tests that are neither IR-specific nor backend-specific
- `tools/unittest/emit/<lang>/`: tests specific to a language backend, where `<lang>` is `cpp,rs,cs,js,ts,go,java,swift,kotlin,rb,lua,scala,php,nim`
- `tools/unittest/ir/`: IR tests for EAST1, EAST2, EAST3, optimizations, and boundary contracts
- `tools/unittest/tooling/`: operational tests such as CLI, parity, manifest, and docs guards
- `tools/unittest/selfhost/`: selfhost generation, diff, and regression tests
- File naming:
...
- Individual runs should remain possible per domain via `python3 -m unittest discover -s tools/unittest/<domain> -p 'test*.py'`.
