...
- Current contracts / manifests / build profiles still explicitly allow exceptions such as `generated compare_artifact + native handwritten canonical`. The remaining concrete example is C# `pathlib`: `src/runtime/cs/native/std/pathlib.cs` still remains the canonical hand-written lane even after `json` moved to the generated-first path.
