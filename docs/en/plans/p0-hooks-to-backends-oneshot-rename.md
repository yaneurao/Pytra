...
- Per user instruction, we will not do phased migration; we will do a one-shot rename to `src/backends/` and converge as top priority.

Goal:
- Standardize the canonical backend implementation path to `src/backends/<lang>/` and remove ambiguity from the `hooks` name.
- Update existing imports/CLI/tests/docs in the same change so no rename-origin breakage remains.

In scope:
- Directory move: `src/hooks/** -> src/backends/**`
...
Acceptance criteria:
- `src/backends/<lang>/` becomes the only canonical location for backend implementations.
- Imports in the repository reference `backends.*` in principle, with no remaining direct `hooks.*` references (except intentional compatibility layers).
...
- [x] [ID: P0-HOOKS-TO-BACKENDS-RENAME-01-S1-01] Inventory the current `src/hooks/**` structure and finalize the 1:1 move map to `src/backends/**`.
...
- [x] [ID: P0-HOOKS-TO-BACKENDS-RENAME-01-S2-02] Bulk-update imports in `src/py2*.py` and compiler/utility code from `hooks.*` to `backends.*`.
- [x] [ID: P0-HOOKS-TO-BACKENDS-RENAME-01-S2-03] Bulk-update imports in `tools/**` and `test/**` to `backends.*` and restore test execution paths.
...
- 2026-03-03: Moved `src/hooks` to `src/backends` in one shot and updated imports in `src/py2*.py` / `src/backends/**` / `tools/**` / `test/**` to `backends.*`.
- 2026-03-03: Fixed rename side effects where blanket replacement turned `hooks.to_dict()` into `backends.to_dict()` in `cpp/cs/rs hooks` implementations and `code_emitter.py`.
...
- 2026-03-03: Chose not to add a compatibility layer (`src/hooks` re-export). Reason: all in-repo references were already updated to `backends.*`, and leaving a compatibility layer would delay convergence to `src/backends` normalization.
