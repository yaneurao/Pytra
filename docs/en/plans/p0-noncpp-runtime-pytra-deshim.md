...
- 2026-03-13: S1-02 sixth bundle renamed the `compat_files` / `compat_modules` schema in `noncpp_runtime_layout_rollout_remaining_contract.py` to `delete_target_files` / `delete_target_modules`. S1-02 remains open because `target_roots=("generated","native","pytra")` and the wave-B helper/iterator names still retain `compat`.
