...
- 2026-03-11: Added `test/unit/ir/test_east_core_parser_behavior_runtime.py`, then moved 12 representative runtime-call / pathlib / json / builtin-lowering / iter-lowering parser-behavior tests out of `test_east_core.py`.
- 2026-03-11: Added `test/unit/ir/test_east_core_parser_behavior_runtime.py`, then moved 12 representative runtime-annotation / builtin-call / pathlib / range / for-iter parser-behavior tests out of `test_east_core.py`. The remaining tests in `test_east_core.py` are now limited to syntax / statements and a few residual source-contract regressions.
