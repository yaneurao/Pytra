...
- 2026-03-11: Added `core_decorator_semantics.py`, then moved `_sh_parse_decorator_head_and_args`, `_sh_is_dataclass_decorator`, `_sh_is_sealed_decorator`, `_sh_is_abi_decorator`, and `_sh_is_template_decorator` out of `core.py`. The decorator cluster is now locked by a dedicated source-contract test.
