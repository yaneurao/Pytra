...
Background:
- `tools/check_py2scala_transpile.py` currently excludes fixtures in `DEFAULT_EXPECTED_FAILS` as `skipped`, so it does not verify that negative cases actually fail.
...
Scope:
- Rework expected-fail operations in `tools/check_py2scala_transpile.py`
...
Acceptance criteria:
- When running `tools/check_py2scala_transpile.py`, known negative cases are evaluated as failure-expected, not simple `skip`.
...
- 2026-03-02: Updated `tools/check_py2scala_transpile.py` from skip-mode to fail-closed verification. Expected failures are always evaluated, and `unexpected pass` / `unexpected error category` / `unexpected error detail` are treated as failures.
