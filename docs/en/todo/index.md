...
- Progress memo: The third `S2-02` bundle is done. `S2-01` is complete with the emitter reduced to a helper-only residual, and regenerating `iter_ops/gif` plus upstreaming the typed empty-bytes lane retires the generated boxed-list seed bucket. The sample `py_append` baseline is now `41 -> 34`. The typed-lane residuals are now 1 emitter bucket plus 2 generated-runtime buckets and 2 sample buckets. Next is shrinking the generated callers and the remaining sample append bucket.
