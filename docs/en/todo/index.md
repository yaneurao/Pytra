...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] Switch active contracts / checkers / spec wording to `generated/native only` and make checked-in `pytra/**` re-entry fail fast. Progress: the third bundle synced the Rust user docs and doc-policy checker to `delete-target debt` wording. Live `pytra` wording still remains in `rollout_remaining_contract`.
