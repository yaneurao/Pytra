...
- [ ] [ID: P4-BACKEND-PARITY-REPRESENTATIVE-ROLLOUT-01] Fill the remaining unsupported support-matrix cells for the representative tier (`cpp/rs/cs`) as a live implementation rollout task (context: [p4-backend-parity-representative-rollout.md](../plans/p4-backend-parity-representative-rollout.md)). Progress: the residual inventory is fixed, the `cpp` baseline lock is confirmed, `rs` has promoted `builtin.type.isinstance`, `builtin.iter.enumerate`, `syntax.control.try_raise`, `syntax.oop.virtual_dispatch`, and the representative stdlib 5-lane set (`json/pathlib/enum/argparse/re`) to `transpile_smoke`, shrinking the Rust residual to `builtin.iter.zip` only; `rs/cs` have promoted the `tuple_assign` / `lambda_basic` / `list_comprehension` lanes to representative smoke, and `cs` has promoted `builtin.type.isinstance` to `transpile_smoke`.
