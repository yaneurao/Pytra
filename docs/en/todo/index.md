...
- [ ] [ID: P0-CPP-RUNTIME-PACKAGING-DESHIM-01] Move C++ runtime packaging to compiler-direct `generated/native` include surfaces and retire `pytra/core` shims from the compiler path (context: [p0-cpp-runtime-packaging-deshim.md](../plans/p0-cpp-runtime-packaging-deshim.md)). Progress: `compiler_headers`, emitter/runtime-emit, build graph, and layout checker are already aligned to direct `generated/native` + `native/core` includes; the remaining work is deciding whether export-time SDK generator shims are still needed.
