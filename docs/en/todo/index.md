...
- Progress memo: [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02] Moved native `CompilerRootDocument::to_legacy_dict()` and `ResolvedBackendSpec::to_legacy_dict()` onto typed-dict conversion, removing explicit `make_object(...)` calls from both `transpile_cli.cpp` and `backend_registry_static.cpp`. `test_py2x_entrypoints_contract.py` now fixes the contract by asserting both native wrappers stay at zero `make_object` lines.
