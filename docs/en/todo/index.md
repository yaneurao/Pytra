...
- [ ] [ID: P0-PYTRA-NES2-CROSSBACKEND-REPRO-01] Promote the Pytra-NES2 repros into a representative cross-backend contract. Progress: `S1-02` promoted `property_method_call` and `list_bool_index` into `test/fixtures/typing/` with assertion-backed semantics. The next step is wiring the same fixtures into representative smoke on every backend. Context: [p0-pytra-nes2-crossbackend-repro-contract.md](../plans/p0-pytra-nes2-crossbackend-repro-contract.md)
