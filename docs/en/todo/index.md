...
- Progress memo: [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-02] Moved native `CompilerRootDocument::to_legacy_dict()` onto typed-dict conversion, removing explicit `make_object(...)` calls from `transpile_cli.cpp`. `test_py2x_entrypoints_contract.py` now fixes the contract by asserting that both native wrappers, together with the already-shrunk `backend_registry_static.cpp`, stay at zero `make_object` lines.
