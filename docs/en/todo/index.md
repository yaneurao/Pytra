...
- [ ] [ID: P0-COLLECTIONS-DEQUE-CPP-ROTATE-01] Lock the representative C++ lane for `collections.deque.rotate()`. Progress: `S2-01` upstreamed `rotate()` / `rotate(n)` to `::std::rotate(...)` bundles. The remaining work is the representative smoke and closeout. Context: [p0-collections-deque-cpp-rotate-support.md](../plans/p0-collections-deque-cpp-rotate-support.md)
