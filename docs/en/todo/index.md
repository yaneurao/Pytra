...
- Progress memo: [ID: P2-COMPILER-TYPED-BOUNDARY-01-S3-01] Added `_sh_make_constant_expr()` and moved `Name` / `Tuple` / `Constant` construction in `_parse_comp_target()`, `_parse_primary()`, and the simple list-comp/tuple lowering paths through helpers.
