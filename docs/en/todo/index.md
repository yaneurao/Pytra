...
- Progress memo: The ninth `S2-02` bundle is done. `S2-01` is complete with the emitter reduced to a helper-only residual, and regenerating `iter_ops/gif` plus upstreaming the typed empty-bytes lane retires the generated boxed-list seed bucket. The sample `py_append` bucket is retired too, `predicates` plus `iter_ops` iterator lowering shrinks the generated generic-index baseline from `46 -> 42`, and the ninth bundle synchronizes the crossruntime residual-caller checker to the current generated `py_runtime_value_*` thin seam while retiring the stale generated object-bridge bucket. The typed-lane residuals are down to 1 emitter bucket plus 1 generated-runtime bucket plus 1 sample bucket. Next is shrinking the generated callers and the remaining sample generic-index bucket.
