...
- [ ] [ID: P0-NONCPP-RUNTIME-PYTRA-DESHIM-01-S1-02] Switch active contracts / checkers / spec wording to `generated/native only` and make checked-in `pytra/**` re-entry fail fast. Progress: second bundle flipped the Rust active contract / layout-guard `pytra` wording to `delete-target debt`.
