...
- [ ] [ID: P0-NONCPP-RUNTIME-LAYOUT-ALIGN-01] Re-layout the `rs/cs` runtimes into a `generated/native` tree that can be compared against C++ at file level (context: [p0-noncpp-runtime-layout-alignment.md](../plans/p0-noncpp-runtime-layout-alignment.md)). Progress: the `rs/cs` trees are already cut over to `native/generated`, and `utils/{png,gif}`, `std/{time,math,os,os_path,glob,pathlib}`, and `built_in/*` have been regenerated there from SoT. The C# std ownership contract now wires `time=generated(std) canonical + native(built_in) backing seam` live while keeping `json=native(std, generated blocked)`, `pathlib/math=native canonical + generated compare artifact`, and `re/argparse/enum=no runtime module` as deferred lanes. The exact `generated/native built_in` inventory and the `src/runtime/cs/pytra/**` duplicate delete-target allowlist are now fixed in contract form. The Rust std ownership contract is fixed as `time/math=native(built_in)`, `pathlib/os/os_path/glob=compare artifact only`, `json blocked`, and `re/argparse/enum=no runtime module`.
