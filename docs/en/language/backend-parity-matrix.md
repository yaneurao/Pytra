...
- Native-backend wildcard relative imports are now closed through [20260314-p0-relative-wildcard-import-native-rollout.md](../plans/archive/20260314-p0-relative-wildcard-import-native-rollout.md). The representative module-graph bundle is locked at `transpile_smoke_locked + module_graph_bundle_transpile` for `go/java/kotlin/lua/nim/php/ruby/scala/swift`, while the single-file direct lane remains `backend_specific_fail_closed` on every backend.
