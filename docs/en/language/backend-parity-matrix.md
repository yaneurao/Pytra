...
- The next rollout handoff lives in [p1-relative-import-native-path-bundle.md](../plans/p1-relative-import-native-path-bundle.md): `rs/cs/js/ts` stay fixed as the `transpile_smoke_locked` baseline, every remaining non-C++ lane keeps `backend_specific_fail_closed`, and the second-wave bundles proceed in the order `locked_js_ts_smoke_bundle -> native_path_bundle(go/nim/swift) -> jvm_package_bundle(java/kotlin/scala)`. In that staged handoff, `go/nim/swift` move under `native_path_bundle_rollout`, `java/kotlin/scala` stay under `remaining_second_wave_rollout_planning`, and `lua/php/ruby` stay deferred behind `defer_until_second_wave_remaining_complete`.
