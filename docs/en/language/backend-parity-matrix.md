...
- The next rollout handoff lives in [p1-relative-import-longtail-bundle.md](../plans/p1-relative-import-longtail-bundle.md): the historical bundle order remains fixed as `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle`. `java/kotlin/scala` now belong to the archived JVM bundle / `transpile_smoke_locked` baseline, and the active bundle is the `lua/php/ruby` `longtail_relative_import_rollout`. The current non-C++ fail-closed lane stays explicitly `backend_specific_fail_closed`, and the long-tail current evidence lane is fixed to `backend_native_fail_closed`. The follow-up rollout bundle and follow-up verification lane are both `none`.
