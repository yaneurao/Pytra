...
- Today `cpp` is `build_run_locked`, `rs/cs/go/js/nim/swift/ts` are `transpile_smoke_locked`, and the evidence lane for `go/nim/swift` is fixed as direct native-emitter function-body transpile smoke. `java/kotlin/lua/php/ruby/scala` remain `not_locked`.
...
- The next rollout handoff lives in [p1-relative-import-native-path-bundle.md](../plans/p1-relative-import-native-path-bundle.md): the bundle order is fixed as `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle`. `rs/cs/js/ts` stay fixed as the `transpile_smoke_locked` baseline, while `go/nim/swift` now keep locked representative smoke but remain the active `native_path_bundle_rollout`. The follow-up `java/kotlin/scala` lane stays on the `jvm_package_bundle` and `remaining_second_wave_rollout_planning`, and `lua/php/ruby` remain deferred behind `defer_until_second_wave_remaining_complete`. The current non-C++ fail-closed lane stays explicitly `backend_specific_fail_closed`.
