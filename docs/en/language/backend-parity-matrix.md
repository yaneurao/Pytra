...
- Today `cpp` is `build_run_locked`, while `rs/cs/go/java/js/kotlin/lua/nim/scala/swift/ts` are `transpile_smoke_locked`. The evidence lane stays fixed as direct native-emitter function-body transpile smoke for `go/nim/swift`, as `package_project_transpile` for the JVM package bundle on `java/kotlin/scala`, and as the representative relative-import alias-rewrite lane for `lua`. Support is still not claimed for `php/ruby`, and the archived long-tail fail-closed bundle keeps their representative baseline at `fail_closed_locked` with `backend_native_fail_closed`.
...
- The next rollout handoff lives in [p1-relative-import-longtail-support.md](../plans/p1-relative-import-longtail-support.md): the historical bundle order remains fixed as `locked_js_ts_smoke_bundle -> native_path_bundle -> jvm_package_bundle`. `java/kotlin/scala` stay in the archived JVM bundle / `transpile_smoke_locked` baseline, while the active `longtail_relative_import_support_rollout` now reports a `mixed_rollout_locked + mixed_backend_evidence` current bundle. `lua` is the current `transpile_smoke_locked` representative lane with `native_emitter_function_body_transpile` evidence, while `php/ruby` remain residual fail-closed lanes with `backend_specific_fail_closed` / `backend_native_fail_closed`. The backend-local focused smoke lanes are fixed as `lua_relative_import_support_rollout_smoke` / `php_relative_import_support_rollout_smoke` / `ruby_relative_import_support_rollout_smoke`, the current smoke-locked backend is `lua`, the remaining rollout backends are `php/ruby`, and the follow-up rollout bundle and follow-up verification lane are both `none`.
