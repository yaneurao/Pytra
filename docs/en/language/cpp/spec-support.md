...
| Basic `bytes` / `bytearray` operations | supported | Supported, including representative `bytes` truthiness (`if payload`, `while payload`, `x if payload else y`). The representative `bytearray` truthiness lane remains a separate follow-up task. | `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/backends/cpp/test_east3_cpp_bridge.py`, `test/fixtures/typing/bytes_basic.py:1`, `test/fixtures/typing/bytes_truthiness.py:1`, `test/fixtures/typing/bytearray_basic.py:1` |
...
| `@dataclass` | supported | Supported. The representative static subset is compile-smoked through `field(default=...)`, `field(default_factory=deque)`, and `field(default_factory=Child)` on `rc<Child>`. Arbitrary callable `default_factory` and full Python dataclasses compatibility remain out of scope. | `test/unit/test_py2cpp_features.py:1519`, `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/ir/test_east_core_parser_behavior_classes.py`, `test/fixtures/stdlib/dataclasses_extended.py:1` |
| `collections.deque[T]` (representative lane) | partial | The C++ type lowering for `deque[T]` is now locked to `::std::deque<T>`, and the dataclass field lane is compile-smoked through `init=False` zero-arg construction and `default_factory=deque`. Full `deque` API compatibility and the general plain-class constructor policy remain out of scope. | `src/backends/cpp/emitter/type_bridge.py`, `src/backends/cpp/emitter/header_builder.py`, `src/runtime/cpp/native/core/py_types.h`, `test/unit/backends/cpp/test_py2cpp_features.py` |
| `str(Path(...))` (representative lane) | supported | In the representative C++ lane, `Path` stringify lowers through `path.__str__()` and no longer falls back to generic `py_to_string(T)`. This is locked with compile/run smoke. | `test/unit/backends/cpp/test_py2cpp_features.py`, `test/unit/backends/cpp/test_cpp_runtime_iterable.py`, `test/fixtures/stdlib/path_stringify.py:1` |
