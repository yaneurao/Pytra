...
目的:
- `docs-ja/todo/archive/YYYYMMDD.md` を基点に、`docs/todo/archive/YYYYMMDD.md` へ同期する定常フローを定義する。
...
- `docs-ja/todo/archive` の日付ファイル集合と `docs/todo/archive` のミラー集合を自動チェックできる

運用手順:
1. `python3 tools/sync_todo_history_translation.py` を実行し、`docs/todo/archive/YYYYMMDD.md` の不足分を作成する。
...
4. `git diff -- docs-ja/todo/archive docs/todo/archive` で差分単位を確認してコミットする。
...
決定ログ:
- 2026-02-22: 初版作成。
- 2026-02-22: `tools/sync_todo_history_translation.py` を追加し、`docs/todo/archive` の日付ファイル雛形と index 同期を自動化した。
