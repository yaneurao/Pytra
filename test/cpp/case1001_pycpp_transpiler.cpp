...
public:
        ...
        for (const auto& stmt : module->body)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        include_lines = include_lines->union(this->_includes_from_import(stmt));
                    }
                    else
                    {
    ...
    vector<string> _helper_functions()
    {
        return {"template <typename T>", "string py_to_string(const T& value)", "{", "    std::ostringstream oss;", "    oss << value;", "    return oss.str();", "}", "", "template <typename T>", "bool py_in(const T& key, const unordered_set<T>& s)", "{", "    return s.find(key) != s.end();", "}", "", "template <typename K, typename V>", "bool py_in(const K& key, const unordered_map<K, V>& m)", "{", "    return m.find(key) != m.end();", "}", "", "template <typename T>", "bool py_in(const T& key, const vector<T>& v)", "{", "    for (const auto& item : v) {", "        if (item == key) {", "            return true;", "        }", "    }", "    return false;", "}", "", "inline void py_print()", "{", "    std::cout << std::endl;", "}", "", "template <typename T>", "void py_print_one(const T& value)", "{", "    std::cout << value;", "}", "", "inline void py_print_one(bool value)", "{", "    std::cout << (value ? \"True\" : \"False\");", "}", "", "template <typename T, typename... Rest>", "void py_print(const T& first, const Rest&... rest)", "{", "    py_print_one(first);", "    ((std::cout << \" \", py_print_one(rest)), ...);", "    std::cout << std::endl;", "}"};
    }
    unordered_set<string> _includes_from_import(ast->stmt stmt)
    {
        unordered_set<string> includes = set();
        ...
        {
            if ((isinstance(stmt, ast->ImportFrom) && stmt->module))
            {
                modules = {stmt->module};
            }
        }
        auto mapping = {{ "math", "#include <cmath>" }, { "pathlib", "#include <filesystem>" }, { "typing", "#include <any>" }, { "dataclasses", "#include <utility>" }};
        for (const auto& mod : modules)
        {
            if (py_in(mod, mapping))
            {
    ...
    string transpile_class(ast->ClassDef cls)
    {
        ...
        auto base = " : public pycs::gc::PyObj";
        if (cls->bases)
        {
        ...
        auto lines = {("class " + py_to_string(cls->name) + py_to_string(base)), "{", "public:"};
        ...
        for (const auto& _for_item : dataclass_fields)
        {
            ...
            {
                lines->extend(this->_indent_block({(py_to_string(field_type) + " " + py_to_string(field_name) + ";")}));
            }
            else
            {
                lines->extend(this->_indent_block({(py_to_string(field_type) + " " + py_to_string(field_name) + " = " + py_to_string(default_value) + ";")}));
            }
        }
        ...
        if ((is_dataclass && dataclass_fields && (!has_init)))
        {
            ...
            lines->extend(this->_indent_block({(py_to_string(cls->name) + "(" + py_to_string(", "->join(ctor_params)) + ")")}));
    ...
    tuple<string, string> _transpile_class_static_assign(ast->Assign stmt)
    {
        ...
        auto field_type = (this->_infer_expr_cpp_type(stmt->value) || "auto");
        return std::make_tuple(("inline static " + py_to_string(field_type) + " " + py_to_string(field_name) + " = " + py_to_string(this->transpile_expr(stmt->value)) + ";"), field_name);
    }
    ...
    string _infer_expr_cpp_type(ast->expr expr)
    {
        if (isinstance(expr, ast->Constant))
        {
    ...
    string transpile_function(ast->FunctionDef fn, bool in_class)
    {
        ...
        if (is_constructor)
        {
            ...
            auto signature = (py_to_string(this->current_class_name) + "(" + py_to_string(", "->join(params)) + ")");
        }
        else
        {
            auto signature = (py_to_string(return_type) + " " + py_to_string(fn->name) + "(" + py_to_string(", "->join(params)) + ")");
        }
    ...
    {
        auto lines = {"int main()", "{"};
        auto body_lines = this->transpile_statements(body, pycs::gc::RcHandle<Scope>::adopt(pycs::gc::rc_new<Scope>(set())));
        lines->extend(this->_indent_block(body_lines));
        lines->extend(this->_indent_block({"return 0;"}));
        lines->append("}");
        return "
    "->join(lines);
    }
        ...
        for (const auto& stmt : stmts)
        {
            ...
            else
            {
                if (isinstance(stmt, ast->Expr))
                {
                    ...
                    lines->append((py_to_string(expr) + ";"));
                }
                else
                {
                    if (isinstance(stmt, ast->AnnAssign))
    ...
    vector<string> _transpile_ann_assign(ast->AnnAssign stmt, pycs::gc::RcHandle<Scope> scope)
    {
        ...
        if ((stmt->value == nullptr))
        {
            auto line = (py_to_string(cpp_type) + " " + py_to_string(name) + ";");
        }
        else
        {
            auto line = (py_to_string(cpp_type) + " " + py_to_string(name) + " = " + py_to_string(this->transpile_expr(stmt->value)) + ";");
        }
    ...
    vector<string> _transpile_assign(ast->Assign stmt, pycs::gc::RcHandle<Scope> scope)
    {
        ...
        if (isinstance(stmt->targets[0], ast->Tuple))
        {
            ...
            auto lines = {("auto " + py_to_string(tmp_name) + " = " + py_to_string(this->transpile_expr(stmt->value)) + ";")};
            for (const auto& _for_item : enumerate(tuple_target->elts, 1))
            {
                ...
                {
                    scope->declared->add(name);
                    lines->append(("auto " + py_to_string(name) + " = std::get<" + py_to_string((i - 1)) + ">(" + py_to_string(tmp_name) + ");"));
                }
                else
                {
                    lines->append((py_to_string(name) + " = std::get<" + py_to_string((i - 1)) + ">(" + py_to_string(tmp_name) + ");"));
                }
            }
            return lines;
        }
    ...
    vector<string> _transpile_for(ast->For stmt, pycs::gc::RcHandle<Scope> scope)
    {
        ...
        auto lines = {("for (const auto& " + py_to_string(target_name) + " : " + py_to_string(this->transpile_expr(stmt->iter)) + ")"), "{"};
        ...
        if ((tuple_target != nullptr))
        {
            for (const auto& _for_item : enumerate(tuple_target->elts, 1))
            {
                auto i = std::get<0>(_for_item);
                auto elt = std::get<1>(_for_item);
                lines->extend(this->_indent_block({("auto " + py_to_string(elt->id) + " = std::get<" + py_to_string((i - 1)) + ">(" + py_to_string(target_name) + ");")}));
    ...
    vector<string> _transpile_try(ast->Try stmt, pycs::gc::RcHandle<Scope> scope)
    {
        ...
        {
            auto ex_type = "std::exception";
            if ((handler->type != nullptr))
            {
                if ((isinstance(handler->type, ast->Name) && (handler->type->id == "Exception")))
                {
            ...
            lines->append(("catch (const " + py_to_string(ex_type) + "& " + py_to_string(ex_name) + ")"));
            lines->append("{");
    ...
    vector<string> _transpile_raise(ast->Raise stmt)
    {
        ...
        if ((isinstance(stmt->exc, ast->Call) && isinstance(stmt->exc->func, ast->Name) && (stmt->exc->func->id == "Exception") && stmt->exc->args))
        {
    ...
    string transpile_expr(ast->expr expr)
    {
        ...
        if (isinstance(expr, ast->Attribute))
        {
            ...
            if ((isinstance(expr->value, ast->Name) && (expr->value->id == "self")))
            {
                return ("this->" + py_to_string(expr->attr));
            }
            if ((isinstance(expr->value, ast->Name) && py_in(expr->value->id, this->class_names)))
            {
        ...
        {
            return ("std::make_tuple(" + py_to_string(", "->join(/* comprehension */ {})) + ")");
        }
    ...
    string _map_annotation(ast->expr annotation)
    {
        ...
        if (isinstance(annotation, ast->Subscript))
        {
            ...
            auto base_map = {{ "list", "vector" }, { "set", "unordered_set" }, { "dict", "unordered_map" }, { "tuple", "tuple" }, { "List", "vector" }, { "Set", "unordered_set" }, { "Dict", "unordered_map" }, { "Tuple", "tuple" }};
    ...
    string _transpile_joined_str(ast->JoinedStr expr)
    {
        ...
        for (const auto& value : expr->values)
        {
            ...
            {
                parts->append(this->_constant(value->value));
            }
            else
            {
                if (isinstance(value, ast->FormattedValue))
                {
                    parts->append(("py_to_string(" + py_to_string(this->transpile_expr(value->value)) + ")"));
                }
                else
                {
                    parts->append("\"\"");
