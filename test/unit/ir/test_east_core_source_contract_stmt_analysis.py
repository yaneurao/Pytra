...
class EastCoreSourceContractStmtAnalysisTest(unittest.TestCase):
    ...
    def test_core_source_routes_stmt_analysis_through_helper_module(self) -> None:
        ...
        self.assertIn("yield_types = _sh_collect_yield_value_types(fn_stmts)", core_text)
        self.assertIn("fn_ret = _sh_infer_return_type_for_untyped_def(fn_ret, fn_stmts)", core_text)
        self.assertIn("docstring, fn_stmts = _sh_extract_leading_docstring(fn_stmts)", core_text)
        self.assertIn("arg_usage_map = _sh_build_arg_usage_map(arg_order, arg_types, fn_stmts)", core_text)
