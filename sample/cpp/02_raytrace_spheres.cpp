...
float64 hit_sphere(float64 ox, float64 oy, float64 oz, float64 dx, float64 dy, float64 dz, float64 cx, float64 cy, float64 cz, float64 r) {
    ...
    if (d < 0.0)
        return -1.0;
    float64 sd = py_to_float64(pytra::std::math::sqrt(d));
        ...
        for (int64 x = 0; x < width; ++x) {
            ...
            for (int64 ay = 0; ay < aa; ++ay) {
                for (int64 ax = 0; ax < aa; ++ax) {
                    float64 fy = (py_to_float64(y) + (py_to_float64(ay) + 0.5) / py_to_float64(aa)) / (py_to_float64(height - 1));
                    float64 fx = (py_to_float64(x) + (py_to_float64(ax) + 0.5) / py_to_float64(aa)) / (py_to_float64(width - 1));
                    float64 sy = 1.0 - 2.0 * fy;
                    float64 sx = (2.0 * fx - 1.0) * (py_to_float64(width) / py_to_float64(height));
                    float64 dx = sx;
                    float64 dy = sy;
                    float64 dz = 1.0;
                    float64 inv_len = py_to_float64(1.0 / pytra::std::math::sqrt(dx * dx + dy * dy + dz * dz));
                    dx *= inv_len;
                    dy *= inv_len;
                    dz *= inv_len;
