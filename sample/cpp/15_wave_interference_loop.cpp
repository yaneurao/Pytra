    ...
    for (int64 t = 0; t < frames_n; ++t) {
        ...
        for (int64 y = 0; y < h; ++y) {
            ...
            for (int64 x = 0; x < w; ++x) {
                int64 dx = x - 160;
                int64 dy = y - 120;
                auto v = pytra::std::math::sin((py_to_float64(x) + py_to_float64(t) * 1.5) * 0.045) + pytra::std::math::sin((py_to_float64(y) - py_to_float64(t) * 1.2) * 0.04) + pytra::std::math::sin((py_to_float64(x + y)) * 0.02 + phase) + pytra::std::math::sin(pytra::std::math::sqrt(dx * dx + dy * dy) * 0.08 - phase * 1.3);
