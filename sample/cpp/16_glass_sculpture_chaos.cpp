...
::std::tuple<float64, float64, float64> refract(float64 ix, float64 iy, float64 iz, float64 nx, float64 ny, float64 nz, float64 eta) {
    ...
    auto cost = pytra::std::math::sqrt(1.0 - sint2);
...
::std::tuple<float64, float64, float64> sky_color(float64 dx, float64 dy, float64 dz, float64 tphase) {
    ...
    r += py_to_float64(0.08 * band);
    g += py_to_float64(0.05 * band);
    b += py_to_float64(0.12 * band);
...
bytes palette_332() {
    ...
    for (int64 i = 0; i < 256; ++i) {
        ...
        int64 g = i >> 2 & 7;
        int64 b = i & 3;
        p[i * 3 + 0] = int64(py_to_float64(255 * r) / py_to_float64(7));
        p[i * 3 + 1] = int64(py_to_float64(255 * g) / py_to_float64(7));
        p[i * 3 + 2] = int64(py_to_float64(255 * b) / py_to_float64(3));
    }
    return bytes(p);
}
...
bytes render_frame(int64 width, int64 height, int64 frame_id, int64 frames_n) {
    float64 t = py_to_float64(frame_id) / py_to_float64(frames_n);
    float64 tphase = 2.0 * pytra::std::math::pi * t;
    float64 cam_r = 3.0;
    float64 cam_x = cam_r * pytra::std::math::cos(py_to_float64(tphase * 0.9));
    float64 cam_y = 1.1 + 0.25 * pytra::std::math::sin(py_to_float64(tphase * 0.6));
    float64 cam_z = cam_r * pytra::std::math::sin(py_to_float64(tphase * 0.9));
    float64 look_x = 0.0;
    float64 look_y = 0.35;
    float64 look_z = 0.0;
    ...
    float64 s0x = 0.9 * pytra::std::math::cos(py_to_float64(1.3 * tphase));
    float64 s0y = 0.15 + 0.35 * pytra::std::math::sin(py_to_float64(1.7 * tphase));
    float64 s0z = 0.9 * pytra::std::math::sin(py_to_float64(1.3 * tphase));
    float64 s1x = 1.2 * pytra::std::math::cos(py_to_float64(1.3 * tphase + 2.094));
    float64 s1y = 0.10 + 0.40 * pytra::std::math::sin(py_to_float64(1.1 * tphase + 0.8));
    float64 s1z = 1.2 * pytra::std::math::sin(py_to_float64(1.3 * tphase + 2.094));
    float64 s2x = 1.0 * pytra::std::math::cos(py_to_float64(1.3 * tphase + 4.188));
    float64 s2y = 0.20 + 0.30 * pytra::std::math::sin(py_to_float64(1.5 * tphase + 1.9));
    float64 s2z = 1.0 * pytra::std::math::sin(py_to_float64(1.3 * tphase + 4.188));
    float64 lr = 0.35;
    float64 lx = 2.4 * pytra::std::math::cos(py_to_float64(tphase * 1.8));
    float64 ly = 1.8 + 0.8 * pytra::std::math::sin(py_to_float64(tphase * 1.2));
    float64 lz = 2.4 * pytra::std::math::sin(py_to_float64(tphase * 1.8));
    bytearray frame = bytearray(width * height);
    float64 aspect = py_to_float64(width) / py_to_float64(height);
    float64 fov = 1.25;
    for (int64 py = 0; py < height; ++py) {
        int64 row_base = py * width;
        float64 sy = 1.0 - 2.0 * (py_to_float64(py) + 0.5) / py_to_float64(height);
        for (int64 px = 0; px < width; ++px) {
            float64 sx = (2.0 * (py_to_float64(px) + 0.5) / py_to_float64(width) - 1.0) * aspect;
            ...
            if (hit_kind == 0) {
                ...
                object glow;
                if (hit_kind == 1) {
                    hx = make_object(cam_x + best_t * dx);
                    hz = make_object(cam_z + best_t * dz);
                    cx = py_to_int64(pytra::std::math::floor(py_to_float64(hx * 2.0)));
                    cz = py_to_int64(pytra::std::math::floor(py_to_float64(hz * 2.0)));
                    ...
                    lxv = make_object(lx - hx);
                    lyv = make_object(ly - -1.2);
                    lzv = make_object(lz - hz);
                    auto __tuple_6 = normalize(py_to_float64(lxv), py_to_float64(lyv), py_to_float64(lzv));
                    ldx = ::std::get<0>(__tuple_6);
                    ldy = ::std::get<1>(__tuple_6);
                    ldz = ::std::get<2>(__tuple_6);
                    ndotl = make_object(::std::max<float64>(static_cast<float64>(ldy), static_cast<float64>(0.0)));
                    auto ldist2 = lxv * lxv + lyv * lyv + lzv * lzv;
                    glow = make_object(8.0 / (1.0 + ldist2));
                    r = py_to_float64(base_r + 0.8 * glow + 0.20 * ndotl);
                    g = py_to_float64(base_g + 0.5 * glow + 0.18 * ndotl);
                    b = py_to_float64(base_b + 1.0 * glow + 0.24 * ndotl);
                } else {
                    cx = 0.0;
                    float64 cy = 0.0;
                    cz = 0.0;
                    float64 rad = 1.0;
                    if (hit_kind == 2) {
                        cx = py_to_float64(s0x);
                        cy = py_to_float64(s0y);
                        cz = py_to_float64(s0z);
                        rad = 0.65;
                    } else {
                        if (hit_kind == 3) {
                            cx = py_to_float64(s1x);
                            cy = py_to_float64(s1y);
                            cz = py_to_float64(s1z);
                            rad = 0.72;
                        } else {
                            cx = py_to_float64(s2x);
                            cy = py_to_float64(s2y);
                            cz = py_to_float64(s2z);
                            rad = 0.58;
                        }
                    }
                    hx = make_object(cam_x + best_t * dx);
                    ...
                    auto cosi = ::std::max<float64>(static_cast<float64>(-dx * nx + dy * ny + dz * nz), static_cast<float64>(0.0));
                    float64 fr = schlick(cosi, 0.04);
                    r = py_to_float64(tr * (1.0 - fr) + sr * fr);
                    g = py_to_float64(tg * (1.0 - fr) + sg * fr);
                    b = py_to_float64(tb * (1.0 - fr) + sb * fr);
                    lxv = make_object(lx - hx);
                    lyv = make_object(ly - hy);
                    lzv = make_object(lz - hz);
                    auto __tuple_12 = normalize(py_to_float64(lxv), py_to_float64(lyv), py_to_float64(lzv));
                    ldx = ::std::get<0>(__tuple_12);
                    ldy = ::std::get<1>(__tuple_12);
                    ldz = ::std::get<2>(__tuple_12);
                    ndotl = make_object(::std::max<float64>(static_cast<float64>(nx * ldx + ny * ldy + nz * ldz), static_cast<float64>(0.0)));
                    ...
                    auto ndoth = ::std::max<float64>(static_cast<float64>(nx * hvx + ny * hvy + nz * hvz), static_cast<float64>(0.0));
                    auto spec = ndoth * ndoth;
                    spec = spec * spec;
                    spec = spec * spec;
                    spec = spec * spec;
                    glow = make_object(10.0 / (1.0 + lxv * lxv + lyv * lyv + lzv * lzv));
            ...
            r = py_to_float64(pytra::std::math::sqrt(clamp01(r)));
            g = py_to_float64(pytra::std::math::sqrt(clamp01(g)));
            b = py_to_float64(pytra::std::math::sqrt(clamp01(b)));
...
void run_16_glass_sculpture_chaos() {
    ...
    for (int64 i = 0; i < frames_n; ++i)
        frames.append(bytes(render_frame(width, height, i, frames_n)));
    pytra::utils::gif::save_gif(out_path, width, height, frames, palette_332(), int64(py_to_int64(6)), int64(py_to_int64(0)));
